//===- entry-trampoline-fast.cpp - B0->B0 fast path -----------------------===//
//
...
/// entry-trampoline.cpp handles A0 and any rewrite that needs
/// instruction patches.
///
//===----------------------------------------------------------------------===//
...
// from utils/entry-trampoline-fast-stub.s, and
...
// internal.h).
#include "entry-trampoline-fast-stub.inc"
...
SmallVector<uint8_t> buildKernelEntryTrampolineFast(uint64_t StubVAddr,
  ...
  // FastEntry*Offset encoding table in internal.h). ScratchSgpr

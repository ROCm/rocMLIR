...
module {
  func @miopen_conv2d_kcyx_nchw_nkhw(%arg0: memref<1024x1024x1x1xf32>, %arg1: memref<128x1024x14x14xf32>, %arg2: memref<128x1024x14x14xf32>) {
    ...
    miopen.gridwise_gemm_v2(%0, %3, %4) {arch = "gfx908", dilations = [1 : i32, 1 : i32], filter_dimension = [1, 1024, 1024, 1, 1], filter_layout = ["g", "kgroup", "cgroup", "y", "x"], input_dimension = [128, 1, 1024, 14, 14], input_layout = ["ni", "g", "cgroup", "hi", "wi"], kernel_algorithm = "v4r4", num_cu = 120 : i32, output_dimension = [128, 1, 1024, 14, 14], output_layout = ["no", "g", "kgroup", "ho", "wo"], padding = [[0 : i32, 0 : i32], [0 : i32, 0 : i32]], strides = [1 : i32, 1 : i32], xdlopsV2 = true, num_group = 1:i32} : memref<1x1024x1024xf32>, memref<1x1024x25088xf32>, memref<1x1024x25088xf32>
    return
  }
  func @main() {

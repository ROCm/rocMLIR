...
struct GlobalLoadToLDSRewritePattern
  ...
  LogicalResult matchAndRewrite(GlobalLoadToLDSOp op,
    ...
    if (isPagedLoad) {
      // Paged load to LDS path: use ROCDL intrinsics directly
      MemRefType srcType = cast<MemRefType>(source.getType());
      Type elemType = srcType.getElementType();
      int64_t elemBytes = elemType.getIntOrFloatBitWidth() / 8;
      ...
      // Get LDS pointer from dest memref
      // First cast dest to LDS address space (3)
      auto ldsType = MemRefType::get(
          cast<MemRefType>(dest.getType()).getShape(),
          cast<MemRefType>(dest.getType()).getElementType(),
          cast<MemRefType>(dest.getType()).getLayout(),
          b.getAttr<gpu::AddressSpaceAttr>(gpu::AddressSpace::Workgroup));
      Value ldsDest =
          b.createOrFold<memref::MemorySpaceCastOp>(loc, ldsType, dest);

      // Get LDS base pointer and add destCoord offset
      Value ldsIndex = destCoords[0];
      Value ldsIndexBytes =
          arith::MulIOp::create(b, loc, ldsIndex, elemBytesVal);
      Value ldsIndexI32 =
          arith::IndexCastOp::create(b, loc, b.getI32Type(), ldsIndexBytes);

      // Create LDS pointer: need to extract base from memref and add offset
      auto ldsPtrType =
          LLVM::LLVMPointerType::get(b.getContext(), /*addressSpace=*/3);
      Value ldsBasePtr =
          memref::ExtractAlignedPointerAsIndexOp::create(b, loc, ldsDest);
      Value ldsBasePtrI64 =
          arith::IndexCastOp::create(b, loc, b.getI64Type(), ldsBasePtr);
      Value ldsBasePtrLlvm =
          LLVM::IntToPtrOp::create(b, loc, ldsPtrType, ldsBasePtrI64);
      // Add offset to LDS pointer
      Value ldsPtr = LLVM::GEPOp::create(b, loc, ldsPtrType, b.getI8Type(),
                                         ldsBasePtrLlvm, ldsIndexI32);

      // Get transfer size in bytes
      Type transferType = op.getTransferType();
      int64_t transferBytes = transferType.getIntOrFloatBitWidth() / 8;
      Value sizeI32 = b.createOrFold<LLVM::ConstantOp>(
          loc, b.getI32Type(), b.getI32IntegerAttr(transferBytes));

      // soffset (scalar offset) and offset are 0
      Value soffset = b.createOrFold<LLVM::ConstantOp>(loc, b.getI32Type(),
                                                       b.getI32IntegerAttr(0));
      Value offset = b.createOrFold<LLVM::ConstantOp>(loc, b.getI32Type(),
                                                      b.getI32IntegerAttr(0));
      // aux flags: 0 for basic loads
      Value aux = b.createOrFold<LLVM::ConstantOp>(loc, b.getI32Type(),
                                                   b.getI32IntegerAttr(0));
      ...
      if (!isAlwaysValid) {
        // Emit conditional load with validity check
        auto ifOp =
            scf::IfOp::create(b, loc, TypeRange{}, valid, /*withElseRegion=*/false);
        b.setInsertionPointToStart(ifOp.thenBlock());

        ROCDL::RawPtrBufferLoadLdsOp::create(b, loc, rsrc, ldsPtr, sizeI32,
                                             voffset, soffset, offset, aux,

...
| **True compute/memory overlap** | ❌ **NOT achieved** - waterfall pattern observed |
| E2E correctness | ✅ Verified (`[1 1 1]`) |

### Root Cause of Waterfall Pattern

The M-loop slicing creates **two COMPUTE (MFMA) clusters**, not a compute+memory cluster pair.
When barriers release, both wave groups proceed to execute MFMA instructions simultaneously.
Since MFMA units are shared hardware, waves **compete and serialize** → waterfall.

**True ping-pong requires:**
...
Restructure the loop body to **interleave compute and memory operations** (compute-first schedule):
```
...
This is a fundamental restructuring of `RockPipeline.cpp`, not just a modification to the
ping-pong pass.
...
### The Path Forward (PARTIALLY COMPLETED)

The implementation has the structural elements but does NOT achieve true ping-pong overlap:

1. ✅ **Triple buffering** - 3 LDS buffers per allocation (prevents buffer conflicts)
2. ✅ **Dot slicing** - M-loop split into 2 halves (creates 2 clusters)
3. ✅ **Phase shift** - cond_barrier before/after loop (staggers wave groups)
4. ✅ **s_barrier** inside loop for cluster boundaries
5. ❌ **True compute/memory overlap** - NOT achieved (see below)

...
// ============================================================
//
// This pass converts rock.attention into the appropriate rock.gridwise_attention
...
class RockAttentionToGridwisePass
    : public rock::impl::RockAttentionToGridwisePassBase<RockAttentionToGridwisePass> {
...
static Attribute getTuningParams(ConversionPatternRewriter &rw, AttentionOp op) {
  Attribute params = op.getParams().value_or(nullptr);
  if (!params) {
    if(StringAttr perfConfigStrAttr = dyn_cast_or_null<StringAttr>(op->getAttr("perf_config"))){
...
LogicalResult
AttentionRewritePattern::matchAndRewrite(AttentionOp op, AttentionOpAdaptor adaptor,
  ...
  if (!isAccel){
    return op.emitError("Currently, attention op is only supported on GPUs with matrix accelerator extentions");
  }
  ...
  queries = normalizeMatrix(queries, rw, loc, !op.getQTransposed(), "gemm0K", "gemm0M");
  keys = normalizeMatrix(keys, rw, loc, op.getKTransposed(), "gemm0K", "gemm0N");
  values = normalizeMatrix(values, rw, loc, !op.getVTransposed(), "gemm1K", "gemm1N");
  ...
  ArrayRef<int64_t> queriesShape = queries.getType().cast<MemRefType>().getShape();
  ArrayRef<int64_t> keysShape = keys.getType().cast<MemRefType>().getShape();
  ArrayRef<int64_t> valuesShape = values.getType().cast<MemRefType>().getShape();
  GemmSize gemm0Size(/*g=*/queriesShape[0], /*m=*/queriesShape[2], /*k=*/queriesShape[1],
                /*n=*/keysShape[2]);
  GemmSize gemm0ExtraPad =
      requiredPadding(params, gemm0Size).value_or(GemmSize{0, 0, 0, 0});
  GemmSize gemm1Size(/*g=*/queriesShape[0], /*m=*/queriesShape[2], /*k=*/valuesShape[1],
                /*n=*/valuesShape[2]);
  GemmSize gemm1ExtraPad =
      requiredPadding(params, gemm1Size).value_or(GemmSize{0, 0, 0, 0});

  queries = padMatrix(queries, rw, loc, "gemm0K", gemm0ExtraPad.k, "gemm0M", gemm0ExtraPad.m);
  keys = padMatrix(keys, rw, loc, "gemm0K", gemm0ExtraPad.k, "gemm0N", gemm0ExtraPad.n);
  values = padMatrix(values, rw, loc, "gemm1K", gemm1ExtraPad.k, "gemm1N", gemm1ExtraPad.n);
  out = padMatrix(out, rw, loc, "gemm1M", gemm1ExtraPad.m, "gemm1N", gemm1ExtraPad.n);
  ...
  int64_t blockSize = waveSize * accelParams.getNPerBlock() * accelParams.getMPerBlock() /
  ...
  rw.create<GridwiseAttentionAccelOp>(
      ...
      keys, 
      values,
      /*TODO(enable scale here once implemented)*/nullptr,
      out,
      op.getArchAttr(),
      op.getFeaturesAttr(),
      blockSizeAttr,

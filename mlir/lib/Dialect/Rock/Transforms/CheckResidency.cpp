...
public:
  LogicalResult checkFunc(gpu::GPUModuleOp mod, LLVM::LLVMFuncOp func) {
    bool hasGlobalSync = rock::HasGlobalSyncAttr::hasOn(func);
...
#define GET_INT_VALUE(var, op, attrT)                                          \
  int64_t var = 0;                                                             \
  if (auto attr = attrT::getOn(op))                                            \
    var = attr.getInt();                                                       \
  else                                                                         \
    return removeBinary()

    GET_INT_VALUE(ldsRequired, func, rock::SharedBufferSizeAttr);
    GET_INT_VALUE(gridSize, func, rock::GridSizeAttr);
    GET_INT_VALUE(blockSize, func, rock::BlockSizeAttr);
...
#define GET_INT_VALUE2(var, dict, name)                                        \
  ...
  if (auto var##Attr = dict.get(name))                                         \
    ...
    auto archAttr = mhal::ArchAttr::lookupOn(mod->getParentOfType<ModuleOp>());
    if (!archAttr) {
      return removeBinary();
    }
    ...
    rock::BlocksPerCUAttr::setOn(func, blocksPerCU);

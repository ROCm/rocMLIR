...
static FailureOr<std::pair<int64_t, int64_t>> computeCopyPerThreadDirectToLDS(
    Value matrix, Type elementType, int64_t copyPerThread, int64_t kPerBlock,
    int64_t dPerBlock, int64_t kpack, int64_t targetBits, Location loc) {
  int64_t copyKPerThread = 0;
  int64_t copyDPerThread = 0;
  if (targetBits < elementType.getIntOrFloatBitWidth())
    return emitError(loc) << "target bits " << targetBits
                          << " smaller than element type bit width "
  ...
  if (copyFastestDimPerThread != inputDimLen) {
    return emitError(loc) << "direct to LDS failed for " << targetBits
                          << " bit load\n";
  }
  ...
  if (kPerBlock < copyKPerThread || dPerBlock < copyDPerThread) {
    return emitError(loc) << "gemmA per thread copy smaller than per"
...
getVectorDim(PatternRewriter &rewriter, Location loc, Value matrix,
  ...
  FailureOr<std::pair<int64_t, int64_t>> maybeCopyDPerThread = failure();
          ...
          rewriter.create<affine::AffineForOp>(loc, 0, kIterationsGemm0, 1);
      {
        ...
        Value wrappedLDSBufferForLoadA =
            accelEmitterPtrGemm0->wrapLDSBufferForLoad(
                ...
                gemm0InMPerThread, "m", false, false, true);
      ...
      // Emit blockwise GEMM 1.
      {
        ...
        if (!doBypassLDSSecondGemm) {
          ...
          wrappedLDSBufferForLoadB = accelEmitterPtrGemm1->wrapLDSBufferForLoad(
              ...
              gemm1InNPerThread, "n", false, false, true);
        }
...
struct GridwiseGemmAccelRewritePattern
  ...
  void generateReadLoop(
      ...
      bool rotateMWithK, bool rotateNWithK, bool directToLDS, bool ldsLayoutKxM,
      bool ldsLayoutKxN) const {
  ...
  LogicalResult matchAndRewrite(GridwiseGemmAccelOp op,
    ...
    if (directToLDS) {
      ...
      std::tie(arrayAForLoad, arrayA) =
          allocBuffer(b, loc, argTypeA, arrayALen);
      std::tie(arrayBForLoad, arrayB) =
          allocBuffer(b, loc, argTypeB, arrayBLen);
    } else {
      ...
      if (singleBuffering) {
        ...
        auto stage2 = b.create<StageOp>(loc, "MMA");
        {
          ...
          blockwiseGemmAccelOp = b.create<BlockwiseGemmAccelOp>(
              ...
              (ldsLayoutConfigA.ldsLayoutKxD ? b.getUnitAttr() : nullptr),
              (ldsLayoutConfigB.ldsLayoutKxD ? b.getUnitAttr() : nullptr),
        ...
        auto stage2 = b.create<StageOp>(loc, "LDSRead");
        {
          ...
          generateReadLoop(
              ...
              ldsLayoutConfigA.ldsLayoutKxD, ldsLayoutConfigB.ldsLayoutKxD);
          b.create<rock::YieldOp>(loc);
        }

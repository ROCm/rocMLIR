...
struct GridwiseGemmAccelRewritePattern
  ...
  LogicalResult matchAndRewrite(GridwiseGemmAccelOp op,
    ...
    if(true) {
        ...
        // Load from LDS.
        int64_t numberOfElementsWrittenPerThread = 128 / destType.getIntOrFloatBitWidth();
        int64_t iter = dataPerThread / numberOfElementsWrittenPerThread;
        //int64_t numberOfRepetitionsPerThread = (mPerBlock*nPerBlock) / (blockSize*numberOfElementsWrittenPerThread);
        //int64_t regsPerThread = numberOfElementsWrittenPerThread*numberOfRepetitionsPerThread;
        Value finalC = gpuAlloc(b, loc, dataPerThread, destType,
                                    AddressSpace::Private);
        
        TopDownTMBuilder tidIterMerge(b, {"tid", "iter"},
                                {op.getBlockSize(), dataPerThread});
        SmallVector<StringRef, 5> throughDims{"tid"};
        tidIterMerge.passThrough(throughDims);
        tidIterMerge.merge(
            {"iter", "numElements"}, {1, 2}, "iter",
            {iter, numberOfElementsWrittenPerThread});
        ...
        tidIterFlatten.unmerge("flattenBlock", 0, {"iter", "tid", "numElements"},
                            {iter, op.getBlockSize(), numberOfElementsWrittenPerThread});
        ...
        b.create<ThreadwiseReadIntoOp>(
            ...
            ValueRange{tid}, /*forceUnroll=*/true, /*useIndexDiffs=*/true);

        // Save to memory
        // Create views as gridwise sub-tile of C
        TopDownTMBuilder tidIterMerge2(
            b, {"g_block", "m_block", "n_block", "tid", "iter"},
            {bidGridLengths[0], bidGridLengths[1], bidGridLengths[2], op.getBlockSize(),
            dataPerThread},
            loc);
        ...
        b.create<ThreadwiseWriteAllOp>(
            loc, finalC, op.getC(), idToMatrixCMaps,
            /*extraIndices=*/

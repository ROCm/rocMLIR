  ...
  if (sourceElemType != destElemType) {
    ...
    if (sourceElemType == b.getF16Type() && destElemType == b.getF32Type()) {
      ...
      if (sourceElemType == sourceType) {
        result = b.create<DataConvertOp>(loc, destType, source);
      } else {
        ...
        for (int64_t i = 0; i < numElements; ++i) {
          ...
          Value converted =
              b.create<DataConvertOp>(loc, destElemType, extracted);
...
struct XdlopsGemmV2RewritePattern : public OpRewritePattern<XdlopsGemmV2Op> {
  ...
  LogicalResult matchAndRewrite(XdlopsGemmV2Op op,
    ...
    if (!IsKReduction) {
      ...
      auto valueA = ilmkb.create<memref::LoadOp>(loc, dataType, op.matrixA(),
                                                 sourceOffsetA);
      ...
      auto valueB = ilnkb.create<memref::LoadOp>(loc, dataType, op.matrixB(),
                                                 sourceOffsetB);
      ...
      for (int64_t i = 0; i < vectorNumber; ++i) {
        ...
        auto mfma =
            loopKb.create<MFMAV2Op>(loc, vectorType, argA, argB, vectorC);

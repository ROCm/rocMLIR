...
class ConvConverter final : public OpConversionPattern<migraphx::ConvolutionOp> {
public:
  using OpConversionPattern<migraphx::ConvolutionOp>::OpConversionPattern;

  Value getZero (Location loc, Type elemType, ConversionPatternRewriter &rewriter) const {
    auto biasTy = RankedTensorType::get({1}, elemType);
    auto zeroAttr = DenseElementsAttr::get(biasTy, rewriter.getZeroAttr(elemType));
    return rewriter.create<mlir::ConstantOp>(loc, zeroAttr);
  }

  Value getRank4TransposeOp (Location loc, Value input, ConversionPatternRewriter &rewriter, SmallVector<int64_t> &permutation) const {
    ...
    SmallVector<int64_t> newShape{inputShape[permutation[0]], inputShape[permutation[1]], inputShape[permutation[2]], inputShape[permutation[3]]};
    Type newTy =
        RankedTensorType::get(newShape, inputTy.getElementType());

    return rewriter.create<tosa::TransposeOp>(loc, newTy, input, permutationValue);
  }
  ...
  matchAndRewrite(migraphx::ConvolutionOp op, ArrayRef<Value> operands,
    ...
    SmallVector<int64_t> newShape{outShape[0], outShape[2], outShape[3], outShape[1]};
    Type newOutTy =
        RankedTensorType::get(newShape, outputTy.getElementType());

    // Construct a new Conv2DOp.
    auto cop = rewriter.create<tosa::Conv2DOp>(
        loc, newOutTy, ValueRange{input_t, filter_t, getZero(loc, elementTy, rewriter)});
    ...
    // transpose the output back to NCHW so that it can match following operators.

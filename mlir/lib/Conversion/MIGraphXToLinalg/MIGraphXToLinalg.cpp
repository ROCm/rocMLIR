...
LogicalResult BackwardConvConverter::matchAndRewrite(
    ...
    ConversionPatternRewriter &rewriter) const {
  // Backward convolution lowering is similar to foward convolution and is lowered in three steps:
  ...
  input = expandGroupDim(rewriter, loc, input, /*isFilter=*/false, /*isBwd=*/true, group, dim);
  filter = expandGroupDim(rewriter, loc, filter, /*isFilter=*/true, /*isBwd=*/ true, group, dim);
...
void mlir::migraphx::populateMIGraphXToLinalgConversionPatterns(
  ...
  patterns
      .add<DotConverter, ElementwiseConverter<migraphx::AddOp, linalg::AddOp>,
           ...
           ConvConverter, BackwardConvConverter>(

...
static void getSizesAndStrides(
...
static void getSizesAndStridesFloat(
    int64_t rank1, StridedMemRefType<float, 5> *filter, int64_t rank2,
    StridedMemRefType<float, 5> *input, int64_t rank3,
    StridedMemRefType<float, 5> *output, void *f_layout, void *i_layout,
    void *o_layout, std::array<int64_t, 5> &fSizes,
    std::array<int64_t, 5> &fStrides, std::array<int64_t, 5> &iSizes,
    std::array<int64_t, 5> &iStrides, std::array<int64_t, 5> &oSizes,
  ...
  getSizesAndStrides(filterSizes, filterStrides, inputSizes, inputStrides,
                     outputSizes, outputStrides, f_layout, i_layout, o_layout,
                     fSizes, fStrides, iSizes, iStrides, oSizes, oStrides);
}

static void getSizesAndStridesInt8(
    int64_t rank1, StridedMemRefType<int8_t, 5> *filter, int64_t rank2,
    StridedMemRefType<int8_t, 5> *input, int64_t rank3,
    StridedMemRefType<int32_t, 5> *output, void *f_layout, void *i_layout,
    void *o_layout, std::array<int64_t, 5> &fSizes,
    std::array<int64_t, 5> &fStrides, std::array<int64_t, 5> &iSizes,
    std::array<int64_t, 5> &iStrides, std::array<int64_t, 5> &oSizes,
    std::array<int64_t, 5> &oStrides) {
...
mcpuConv2dFloat(int64_t rank1, void *f_ptr, int64_t rank2, void *i_ptr,
  ...
  getSizesAndStridesFloat(rank1, filter, rank2, input, rank3, output, f_layout,
                          i_layout, o_layout, filterSizes, filterStrides,
                          inputSizes, inputStrides, outputSizes, outputStrides);
...
mcpuConv2dInt8(int64_t rank1, void *f_ptr, int64_t rank2, void *i_ptr,
  ...
  getSizesAndStridesInt8(rank1, filter, rank2, input, rank3, output, f_layout,
                         i_layout, o_layout, filterSizes, filterStrides,
                         inputSizes, inputStrides, outputSizes, outputStrides);
  // Perform forward convolution
  for (int64_t g = 0; g < outputSizes[0]; g++)
...
extern "C" void mcpuConv2dBwdWeightInt8(
    int64_t rank1, void *f_ptr, int64_t rank2, void *i_ptr, int64_t rank3,
    void *o_ptr, int64_t rank4, void *f_layout, int64_t rank5, void *i_layout,
    int64_t rank6, void *o_layout, int32_t stride_h, int32_t stride_w,
    int32_t padding_h_l, int32_t padding_h_r, int32_t padding_w_l,
      ...
      for (int64_t c = 0; c < filterSizes[2]; c++)
        for (int64_t y = 0; y < filterSizes[3]; y++)
          for (int64_t x = 0; x < filterSizes[4]; x++) {
            ...
            for (int64_t n = 0; n < outputSizes[1]; n++)
              for (int64_t out_h = 0; out_h < outputSizes[3]; out_h++)
                for (int64_t out_w = 0; out_w < outputSizes[4]; out_w++) {
                  ...
                  if (in_h >= 0 && in_h < inputSizes[3] && in_w >= 0 &&
                      in_w < inputSizes[4])
                    acc += (int32_t)(inputAllocated[g * inputStrides[0] +
                                                    n * inputStrides[1] +
                                                    c * inputStrides[2] +
                                                    in_h * inputStrides[3] +
                                                    in_w * inputStrides[4]] *
            ...
            filterAllocated[g * filterStrides[0] + k * filterStrides[1] +
                            c * filterStrides[2] + y * filterStrides[3] +
                            x * filterStrides[4]] = (int8_t)acc;
          }
}

extern "C" void
mcpuConv2dBwdDataInt8(int64_t rank1, void *f_ptr, int64_t rank2, void *i_ptr,
                      int64_t rank3, void *o_ptr, int64_t rank4, void *f_layout,
      ...
      for (int64_t c = 0; c < inputSizes[2]; c++)
        for (int64_t in_h = 0; in_h < inputSizes[3]; in_h++)
          for (int64_t in_w = 0; in_w < inputSizes[4]; in_w++) {
              ...
              for (int64_t y = 0; y < filterSizes[3]; y++)
                for (int64_t x = 0; x < filterSizes[4]; x++) {
                  ...
                  if (out_h_tmp % stride_h == 0 && out_w_tmp % stride_w == 0 &&
                      out_h >= 0 && out_h < outputSizes[3] && out_w >= 0 &&
                      out_w < outputSizes[4])
                    acc += (int32_t)(filterAllocated[g * filterStrides[0] +
                                                     k * filterStrides[1] +
                                                     c * filterStrides[2] +
                                                     y * filterStrides[3] +
                                                     x * filterStrides[4]] *
            ...
            inputAllocated[g * inputStrides[0] + n * inputStrides[1] +
                           c * inputStrides[2] + in_h * inputStrides[3] +
                           in_w * inputStrides[4]] = (int8_t)acc;
          }
}

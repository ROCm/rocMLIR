...
def advancedNodeCheck(Map params) {
    ...
    ['ls /dev/kfd', 'ls /dev/dri'].each { cmd ->
    ...
    if (env.NODE_LABELS && !env.NODE_LABELS.contains('build-only')) {
        ...
        if (doGPUcheck) {
            timeout(time: 5, unit: 'MINUTES', activity: true) {
                ...
                if (n == 0) {
                    throw NodeCheckFailedException.newInstance("No GPUs detected on ${env.NODE_NAME}")
                }
...
String dockerArgs() {
    // Helper to run a shell command and get its stdout
    def run = { cmd -> sh(script: cmd, returnStdout: true).trim() }

    // Enumerate /dev/dri/renderD* devices if they exist
    ...
    return """
        ...
        ${kfdFlg} \
        ${renderFlags} \
        --group-add ${videoGid} --group-add ${renderGid} \

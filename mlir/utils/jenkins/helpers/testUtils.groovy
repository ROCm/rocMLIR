...
void runShardedE2E(int jobsPerGpu) {
    dir('build') {
        buildUtils.shStrict "python3 ./bin/run_e2e_multigpu.py --build-dir . " +
...
void build_fixedE2ETests(String codepath) {
    ...
    buildUtils.buildProject("check-mlir-build-only check-rocmlir-build-only ci-performance-scripts${params.nightly ? ' hipblaslt-benchmark-driver' : ''}", """

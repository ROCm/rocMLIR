...
def printPerformance(mlir_df, miopen_df):
    ...
    df['Speedup'] = df['TFlops'] / df['MIOpen TFlops']
    df.to_csv(reportUtils.PERF_REPORT_FILE, index=False)

    COLUMNS_TO_AVERAGE = ['TFlops', 'MIOpen TFlops', 'Speedup']
    means = reportUtils.geoMean(df[COLUMNS_TO_AVERAGE])
    means = pd.Series(means, index=COLUMNS_TO_AVERAGE)
    means.name = "Geo. mean"
    means = pd.DataFrame(means).T

    means[['TFlops', 'MIOpen TFlops']].to_csv(reportUtils.PERF_STATS_REPORT_FILE, index=False)

    with open("MLIR_vs_MIOpen.html", 'w') as htmlOutput:
        reportUtils.htmlReport(df, means, "MLIR vs. MIOpen performance", htmlOutput)

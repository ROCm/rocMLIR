...
def runConfigWithMIOpenDriver(commandLine):
    # remove the result file generated by rocprof in previous benchmarking
    os.system("rm "+BENCHMARKING_RESULT_FILE_NAME)
    out=subprocess.run(f'printenv', shell=True)
    os.system("echo");
    out=subprocess.run(f'ldd ../MIOpen/build/bin/MIOpenDriver', shell=True)
...
def printPerformance(mlirFileName, miopenFileName):
    with open(mlirFileName, 'r') as mlirOutput:
        ...
        mlirFlops = [','+row.rstrip().split(',')[-1] for row in mlirResults]
    ...
    # Add geometric means to the table
    mlirGeoMean = geo_mean(mlirFlops)
    miopenGeoMean = geo_mean(miopenFlops)

    means = [' ']*len(header.split(',')[0:-2])
    means.append(str(mlirGeoMean))
    means.append(str(miopenGeoMean))
    table.add_row(means)

...
MLIR_ROCM_RUNNER_ARGS = ['--shared-libs=./external/llvm-project/llvm/lib/libmlir_rocm_runtime.so,./lib/libconv-validation-wrappers.so,./external/llvm-project/llvm/lib/libmlir_runner_utils.so', '--entry-point-result=void']
...
def runConfigWithMLIR(config: ConvConfiguration):
...
def benchmarkMIOpen(commandLine, xdlops, envs=dict()):
    ...
    nanoSeconds = runConfigWithMIOpenDriver(commandLine, envs)
    return config.tableEntry(nanoSeconds)

...
usage: quickTunerStat.py [-h] [--input-dir INPUT_DIR] [--qt-file QT_FILE] [--perfconfig-file PERFCONFIG_FILE] [--dtype {f32,f16,i8}] --gemm-configs GEMM_CONFIGS --data DATA [--method {data,tuner}] [--rank]
                         [--rocmlir ROCMLIR]
...
optional arguments:
  ...
  --input-dir INPUT_DIR
                        Directory of config files generated by quickTuner
  --qt-file QT_FILE     methodname_type.qt file
  --perfconfig-file PERFCONFIG_FILE
                        Perf config formatted file
  --dtype {f32,f16,i8}  Datatype to be used with perfconfig file (required when --perfconfig-file is specified
  --gemm-configs GEMM_CONFIGS
                        File path for gemm-configs
  --data DATA           File path for datafile from quickTunerPreproc.py
  --method {data,tuner}
                        Method for testing the produced files
  --rank                Rank results
  --rocmlir ROCMLIR     Path to rocmlir script

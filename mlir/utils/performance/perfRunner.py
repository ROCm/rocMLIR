...
class PerfConfiguration:
    ...
    @classmethod
    def fromCommandLine(cls, argv, arch, numCU) -> 'self':
        raise NotImplementedError()

    @classmethod
    def benchmarkExternal(cls, commandLine, paths: Paths, arch, numCU, envs=dict()):
        raise NotImplementedError()
...
def benchmarkMLIR(commandLine, confClass, paths: Paths, arch, numCU, tuningDb: MaybeTuningDb, rocmlir_gen_flags, debug=False):
    ...
    if tuningDb:
        if (arch, configStr) in tuningDb:
            ...
            if debug:
                print("Tuning DB doesn't contain this config\n")
            return config.tableEntry(np.nan)

    runConfigWithMLIR(config, paths, rocmlir_gen_flags, debug)
    # get nanoseconds from rocprof output.
    nanoSeconds = getNanoSeconds(BENCHMARKING_RESULT_FILE_NAME)
    return config.tableEntry(nanoSeconds)

#Generate MLIR vs. MIOpen or rocBLAS performance results
def generatePerformanceResults(configs, confClass, paths: Paths, arch, numCU, tuningDb: MaybeTuningDb, rocmlir_gen_flags, debug=False):
    # Never pass tuning DB to this run
    if debug:
      print("Benchmark UnTuned kernels\n")
    mlir_df = pd.DataFrame(benchmarkMLIR(testVector.split(sep=' '), confClass, paths, arch, numCU, None, rocmlir_gen_flags, debug)
        for testVector in configs)
    tuned_df = None
    if tuningDb:
        print("Benchmark Tuned kernels\n")
        tuned_df = pd.DataFrame(benchmarkMLIR(testVector.split(sep=' '), confClass, paths, arch, numCU, tuningDb, rocmlir_gen_flags, debug)
            for testVector in configs)
...
def main(args=None):
    ...
    if parsed_args.batch_all:
        ...
        generatePerformanceResults(configs, confClass, paths, arch, numCU, tuningDb, rocmlir_gen_flags, parsed_args.debug)
        ...
        if parsed_args.batch_mlir:
            df = pd.DataFrame(benchmarkMLIR(testVector.split(sep=' '), confClass, paths, arch, numCU, tuningDb, rocmlir_gen_flags, parsed_args.debug) for testVector in configs)
            ...
            if not parsed_args.mlir_build_dir:
                ...
                df = pd.DataFrame([benchmarkMLIR(parsed_args.config, confClass, paths, arch, numCU, tuningDb, rocmlir_gen_flags, parsed_args.debug)])

...
def find_mlir_build_dir() -> str:
    ...
    if not rocmlir_gen_path:
        ...
        rocmlir_gen_path = glob.glob(search_root + '/**/bin/rocmlir-gen',
                                     recursive=True)
...
def create_paths(config_file_path, mlir_build_dir_path) -> Paths:
    ...
    if mlir_build_dir_path:
        ...
        llvm_bin_dir = str((Path(mlir_build_dir_path) /
                            'external/llvm-project/llvm/bin').resolve())
...
def runPipeline(proc_specs):
    ...
    try:
        ...
        print(
            f"Failing pipeline:  {' | '.join([' '.join(proc) for proc in proc_specs])}"
        )
        outs, errs = p.communicate()
    return outs, False
    ...
    if fileName:
        with open(fileName, 'r') as configFile:
            ...
            for direction, datatype, layout, line in \
                ...
                if datatype == 'convfp8':
                    unsupported_chips = {
                        'gfx908', 'gfx90a', 'gfx942', 'gfx1030', 'gfx1101'
                    }
                    if getChip() in unsupported_chips:
                        continue

                # Skip int8 non-fwd convolutions
                if (datatype == 'convint8' or
                        datatype == 'convfp8') and direction != '-F 1':
                    continue
...
class ConvConfiguration(PerfConfiguration):
    TABLE_COLUMNS = reportUtils.CONV_TEST_PARAMETERS + ['LDSBankConflict'
                                                       ] + ['TFlops']
    ...
    def computeTFlops(self, ns):
        ...
        return (2.0 * self.n * (self.c // self.group) * self.k * self.ho *
                self.wo * self.y * self.x) / (float(ns) * 1e-9) / 1e12

    def tableEntry(self, nanoSeconds):
        # Future(kdrewnia): This can just be a dict literal on Python 3.7+
        bankConflict = getBankConflict(
            getProfilerOutputPath(self.arch, BENCHMARKING_METRICS_FILE_NAME))
        result = OrderedDict()
        values = [
            self.direction, self.dataType, self.chip, self.numCU,
            self.filterLayout, self.inputLayout, self.outputLayout, self.n,
            self.c, self.hi, self.wi, self.k, self.y, self.x, self.dilationH,
            self.dilationW, self.convStrideH, self.convStrideW, self.paddingH,
            self.paddingW, self.perfConfig, bankConflict,
    ...
    def generateMlirDriverCommandLine(self, rocmlir_gen_flags):
        ...
        result = ' '.join([
            ...
            str(self.numCU), '--fil_layout', self.filterLayout, '--in_layout',
            self.inputLayout, '--out_layout', self.outputLayout, '--batchsize',
            ...
            str(self.convStrideW), '--padding_h',
            str(self.paddingH), '--padding_w',
            str(self.paddingW), '--groupsize',
    ...
    def fromCommandLine(cls, argv, arch, numCU):
        ...
        try:
            ...
            opts, _ = getopt.getopt(
                argv[1:], "F:f:I:O:n:c:H:W:k:y:x:p:q:l:j:u:v:g:m:t:")
        ...
        return cls(dataType, direction, filterLayout, inputLayout,
                   outputLayout, n, c, hi, wi, k, y, x, convStrideH,
                   convStrideW, paddingH, paddingW, dilationH, dilationW,
                   group, arch, numCU)
    ...
    def __init__(self, dtype: str, direction: str, filterLayout: str,
                 inputLayout: str, outputLayout: str, n: int, c: int, hi: int,
                 wi: int, k: int, y: int, x: int, convStrideH: int,
                 convStrideW: int, paddingH: int, paddingW: int,
        ...
        self.ho = math.floor(
            (self.hi + self.paddingH * 2 -
             (self.y - 1) * self.dilationH - 1) / self.convStrideH) + 1
        self.wo = math.floor(
            (self.wi + self.paddingW * 2 -
    ...
    def benchmarkExternal(cls, commandLine, paths: Paths, arch, numCU):
        if os.path.exists(
                getProfilerOutputPath(arch, BENCHMARKING_METRICS_FILE_NAME)):
            os.remove(
                getProfilerOutputPath(arch, BENCHMARKING_METRICS_FILE_NAME))
        ...
        with open(fileName, 'r') as configFile:
            ...
            for datatype, transA, transB, line in \
                ...
                if "-out_datatype" not in line:
                    outDataTypeString = "-out_datatype " + outDataTypeMap.get(
                        datatype, datatype) + " "
...
class GemmConfiguration(PerfConfiguration):
    TABLE_COLUMNS = reportUtils.GEMM_TEST_PARAMETERS + ['LDSBankConflict'
                                                       ] + ['TFlops']

    def computeTFlops(self, ns):
        # NaN will propagate as expected
        # Repeats are handled by the fact that we're using avarageNs
        return (2.0 * self.g * self.m * self.k * self.n) / (float(ns) *
                                                            1e-9) / 1e12
    ...
    def generateMlirDriverCommandLine(self, rocmlir_gen_flags):
        result = ' '.join([
            ...
            str(self.m), '-k',
            str(self.k), '-n',
            str(self.n), f"-transA={self.transA}", f"-transB={self.transB}",
            '--kernel-repeats',
            str(MLIR_N_REPEATS), f"--perf_config={self.perfConfig}"
        ])
...
class ConvGemmConfiguration(PerfConfiguration):
    ...
    def computeTFlops(self, ns):
        ...
        first_conv_flops = 2.0 * self.n * (
            self.c //
            self.group) * self.k * self.ho * self.wo * self.y * self.x
        first_gemm_m = self.k
    ...
    def tableEntry(self, nanoSeconds):
        result = {}
        values = [
            self.dataType, self.chip, self.numCU, self.filterLayout,
            self.inputLayout, self.transC, self.transO, self.n, self.c,
            self.hi, self.wi, self.k, self.y, self.x, self.o, self.dilationH,
            self.dilationW, self.convStrideH, self.convStrideW, self.paddingH,
            self.paddingW, self.perfConfig,
            self.computeTFlops(nanoSeconds)
        ]
        assert (len(self.TABLE_COLUMNS) == len(values))
        for k, v in zip(self.TABLE_COLUMNS, values):
    ...
    def generateMlirDriverCommandLine(self, rocmlir_gen_flags):
        result = ' '.join([
            '-operation', 'conv_gemm', '-t', self.dataType, '--arch',
            self.arch, f'--num_cu={self.numCU}',
            f'--fil_layout={self.filterLayout}',
            f'--in_layout={self.inputLayout}', f'--transC={self.transC}',
            f'--transO={self.transO}', f'--batchsize={self.n}',
            f'--in_channels={self.c}', f'--in_h={self.hi}',
            f'--in_w={self.wi}', f'--out_channels={self.k}',
            f'--fil_h={self.y}', f'--fil_w={self.x}',
            f'--dilation_h={self.dilationH}', f'--dilation_w={self.dilationW}',
            f'--conv_stride_h={self.convStrideH}',
            f'--conv_stride_w={self.convStrideW}',
            f'--padding_h={self.paddingH}', f'--padding_w={self.paddingW}',
    ...
    def fromCommandLine(cls, argv, arch, numCU):
        ...
        for i in range(0, len(argv), 2):
            ...
            if opt.endswith("-t"):
                ...
                raise ValueError(
                    f"Unknown conv+gemm config argument {opt} -> {val}")
        for v in [
                dtype, n, c, hi, wi, k, y, x, o, convStrideH, convStrideW,
                paddingH, paddingW, dilationH, dilationW, group, filterLayout,
                inputLayout, transC, transO
        ]:
            if v is None:
                raise ValueError("Incomplete conv+gemm configuration")

        return cls(dtype, filterLayout, inputLayout, transC, transO, n, c, hi,
                   wi, k, y, x, o, convStrideH, convStrideW, paddingH,
                   paddingW, dilationH, dilationW, group, arch, numCU,
                   perf_config)
    ...
    def toCommandLine(self):
        return (
            ...
            f"-y {self.y} -x {self.x} -p {self.paddingH} -q {self.paddingW} " +
            f"-u {self.convStrideH} -v {self.convStrideW} -l {self.dilationH} "
            + f"-j {self.dilationW} -g {self.group}" + f"-gemmO {str(self.o)}")
...
class GemmGemmConfiguration(PerfConfiguration):
    ...
    def tableEntry(self, nanoSeconds):
        result = {}
        values = [
            self.dataType, self.chip, self.numCU, self.transA, self.transB,
            self.transC, self.transO, self.g, self.m, self.k, self.n, self.o,
            self.perfConfig,
            self.computeTFlops(nanoSeconds)
        ]
        assert (len(self.TABLE_COLUMNS) == len(values))
    ...
    def generateMlirDriverCommandLine(self, rocmlir_gen_flags):
        result = ' '.join([
            '-operation', 'gemm_gemm', '-t', self.dataType, '--arch',
            self.arch, '--num_cu',
            str(self.numCU), '-g',
            str(self.g), '-m',
            str(self.m), '-k',
            str(self.k), '-n',
            str(self.n), '-gemmO',
            str(self.o), f"-transA={self.transA}", f"-transB={self.transB}",
            f"-transC={self.transC}", f"-transO={self.transO}",
            '--kernel-repeats',
            str(MLIR_N_REPEATS), f"--perf_config={self.perfConfig}"
        ])
        result += ' '
...
class AttentionConfiguration(PerfConfiguration):
    ...
    def tableEntry(self, nanoSeconds):
        result = {}
        values = [
            self.dataType, self.chip, self.numCU, self.transQ, self.transK,
            self.transV, self.transO, self.causal, self.return_lse,
    ...
    def generateMlirDriverCommandLine(self,
        ...
        result = ' '.join([
            '-operation', 'attention', '-t', self.dataType, '--arch',
            self.arch, '--num_cu',
            str(self.numCU), '-g',
            ...
            f"-transO={self.transO}", f"-causal={self.causal}",
    ...
    def fromCommandLine(cls, argv, arch, numCU):
        ...
        return cls(dtype, g, seq_len_q, seq_len_k, num_heads_q, num_heads_kv,
                   head_dim_qk, head_dim_v, with_attn_scale, with_attn_bias,
                   transQ, transK, transV, transO, causal, return_lse,
                   split_kv, arch, numCU, perf_config)
...
class RocBLASGemmConfig(GemmConfiguration):
    ...
    def benchmarkExternal(cls, commandLine, paths: Paths, arch, numCU):
        ...
        if os.path.exists(
                getProfilerOutputPath(arch, BENCHMARKING_STATS_FILE_NAME)):
            os.remove(getProfilerOutputPath(arch,
                                            BENCHMARKING_STATS_FILE_NAME))
...
def runConfigWithMLIR(config: PerfConfiguration, paths: Paths, arch, rocmlir_gen_flags, use_rocprof=False, debug=True):
    # remove the result file generated by rocprof in previous benchmarking
    if os.path.exists(getProfilerOutputPath(arch,
                                            BENCHMARKING_STATS_FILE_NAME)):
    ...
    if not use_rocprof:
        ...
        outs, noerr = runPipeline([rocmlirGenCommand.split(), tuningDriverCommand])
        if noerr:
...
def benchmarkMLIR(commandLine, confClass, paths: Paths, arch, numCU, tuningDb: MaybeTuningDb, rocmlir_gen_flags, use_rocprof=False):
    ...
    nanoSeconds = runConfigWithMLIR(config, paths, arch, rocmlir_gen_flags, use_rocprof)
    return config.tableEntry(nanoSeconds)


#Generate MLIR vs. MIOpen or rocBLAS performance results
def generatePerformanceResults(configs, confClass, paths: Paths, arch, numCU, tuningDb: MaybeTuningDb, quickTuningDb: MaybeTuningDb, rocmlir_gen_flags, use_rocprof=False):
    # Never pass tuning DB to this run
    mlir_df = pd.DataFrame(benchmarkMLIR(testVector.split(sep=' '), confClass, paths, arch, numCU, None, rocmlir_gen_flags, use_rocprof)
        for testVector in configs)
    tuned_df = None
    if tuningDb:
        tuned_df = pd.DataFrame(benchmarkMLIR(testVector.split(sep=' '), confClass, paths, arch, numCU, tuningDb, rocmlir_gen_flags, use_rocprof)
            for testVector in configs)
    quick_tuned_df = None
    if quickTuningDb:
        quick_tuned_df = pd.DataFrame(benchmarkMLIR(testVector.split(sep=' '), confClass, paths, arch, numCU, quickTuningDb, rocmlir_gen_flags, use_rocprof)
            for testVector in configs)

    external_df = pd.DataFrame(
        confClass.benchmarkExternal(testVector.split(
            sep=' '), paths, arch, numCU) for testVector in configs)

    externalName = confClass.EXTERNAL_NAME
    df = mlir_df.merge(external_df,
                       on=confClass.TABLE_COLUMNS[:-2],
    ...
    if tuned_df is not None:
        ...
        df = df.merge(tuned_df,
                      on=confClass.TABLE_COLUMNS[:-3],
                      suffixes=('', ' (tuned)'))
        df.drop(columns=['PerfConfig'], inplace=True)
        df.rename(columns={
    ...
    if tuned_df is not None and quick_tuned_df is not None:
        df["Quick Tuned/Tuned"] = df['Quick Tuned MLIR TFlops'] / df[
            'Tuned MLIR TFlops']
    chip = GFX_CHIP_RE.search(arch).group(0)
...
def runFusionKernel(filename, rocmlirGenArgs, paths: Paths):
    ...
    profilerCommand = [ROCPROF] + getMetricArgsForRocprof(chip) + [
        '--kernel-trace', '--stats', '-o', BENCHMARKING_RESULT_FILE_NAME
...
def benchmarkFusionKernels(test_dir, paths: Paths, arch, numCU, tuningDb: MaybeTuningDb, use_rocprof=False):
    ...
    for test in allTests:
        ...
        nanoSeconds = runConfigWithMLIR(config, paths, arch, '', use_rocprof)
...
def foundExternalTool(paths: Paths,
                      opType: Operation,
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        ...
        description=
        "A test runner script for MIOpen and MLIR-based kernel generator",
        allow_abbrev=False,
    )
    ...
    parser.add_argument("--op",
                        "--operation",
                        choices=[
                            'conv', 'gemm', 'fusion', 'attention', 'gemm_gemm',
                            'conv_gemm'
                        ],
                        default='conv',
    ...
    mutex_arg_group.add_argument("--tuning",
                                 action="store_true",
    ...
    mutex_arg_group.add_argument(
        ...
        action="store_true",
        help=
        "CSV batch benchmarking with MLIR and external reference (defalut on no args)"
    )
    ...
    parser.add_argument("-o",
                        type=str,
                        default=chip + '_' +
                        date.today().strftime("perf.%m%d%y"),
    ...
    if parsed_args.batch_all:
        ...
        generatePerformanceResults(configs, confClass, paths, arch, numCU, tuningDb, quickTuningDb, rocmlir_gen_flags, parsed_args.use_rocprof)
        ...
        if not parsed_args.mlir_build_dir:
            ...
            benchmarkFusionKernels(parsed_args.test_dir, paths, arch, numCU, tuningDb, parsed_args.use_rocprof)
    else:
        if parsed_args.batch_mlir:
            df = pd.DataFrame(benchmarkMLIR(testVector.split(sep=' '), confClass, paths, arch, numCU, tuningDb, rocmlir_gen_flags, parsed_args.use_rocprof) for testVector in configs)
            ...
            df = pd.DataFrame([
                confClass.benchmarkExternal(parsed_args.config, paths, arch,
                                            numCU)
            ])
        else:
            ...
            if not parsed_args.mlir_build_dir:
                ...
                if parsed_args.config:
                    df = pd.DataFrame([benchmarkMLIR(parsed_args.config, confClass, paths, arch, numCU, tuningDb, rocmlir_gen_flags, parsed_args.use_rocprof)])
                else:
                    df = pd.DataFrame([benchmarkMLIR(config.split(), confClass, paths, arch, numCU, tuningDb, rocmlir_gen_flags, parsed_args.use_rocprof) for config in configs])

...
class RocBLASGemmConfig(GemmConfiguration):
    ...
    def benchmarkExternal(cls, commandLine, paths: Paths, arch, numCU, envs=dict()):
        ...
        profilerCommand = [ROCPROF,'-i', ROCMLIR_INPUT_METRICS_FILE_NAME, '--stats', \
...
def runConfigWithMLIR(config: PerfConfiguration, paths: Paths, rocmlir_gen_flags, debug=True):
    ...
    profilerCommand = [ROCPROF, '-i', ROCMLIR_INPUT_METRICS_FILE_NAME, '--stats', '-o', BENCHMARKING_METRICS_FILE_NAME, paths.mlir_paths.cpu_runner_path] + mlir_cpu_runner_args

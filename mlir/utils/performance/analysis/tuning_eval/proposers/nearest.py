...
from ..features import distance_features, problem_features
...
class NearestKnownProposer(ConfigProposer):
    ...
    def fit(self, train: Corpus) -> None:
        ...
        for key in train.keys():
            ...
            for problem_key in train.problem_keys(key):
                raw_vecs.append(self._raw_vec(train.sig(key, problem_key)))
    ...
    def propose(self, sig: ProblemSig, budget: int) -> List[str]:
        ...
        if idxs is None:
            idxs = self._nearest_idxs(sig, problem_map)
    ...
    def _nearest_idxs(self, sig: ProblemSig, problem_map: Dict[int, List[int]]) -> List[int]:
        index = self._index.get(sig.table_key)
        if not index:
            return []
        mean, std = self._stats[sig.table_key]
        query = self._standardize(self._raw_vec(sig), mean, std)
    ...
    def _raw_vec(self, sig: ProblemSig) -> List[float]:
        feats = problem_features(sig)
        return [feats[name] for name in distance_features(sig.op)]

...
class FileWriter():
    ...
    def replace_section(self, file_path, begin_marker, end_marker,
                        new_content):
        """
    ...
    def init_inc_file(self, file_path):
        ...
        ops = ["GEMM", "CONV"]
        architectures = [
            "gfx908", "gfx90a", "gfx942", "gfx950", "gfx1030", "gfx1100",
            "gfx1200"
        ]
                ...
                for datatype in datatypes:
                    for op in ops:
                        for arch in architectures:
                            if self.get_instruction_type(
                                    arch, datatype) == instruction_type:
                                file.write(
                                    f"// BEGIN_{op}_{instruction_type}_{datatype}_{arch}_DEFS\n\n"
                                )
                                file.write(
                    ...
                    for op in ops:
                        for arch in architectures:
                            if self.get_instruction_type(
                                    arch, datatype) == instruction_type:
                                file.write(
                                    f"// BEGIN_{op}_{instruction_type}_{datatype}_{arch}_DECS\n\n"
                                )
                                file.write(
    ...
    def get_init_params_declaration(self, arch, dtype, op):
        ...
        return (
            f"static const InitParams{accel_type} {init_params}[{n_init_params}]",
...
class PerfConfigsFinder():
    ...
    def find(self):
        ...
        for data_type in unique_data_types:
            ...
            perfconfig_to_index = {
                perfconfig: idx for idx, perfconfig in enumerate(perfconfigs)
            }
            ...
            selected_configs = [
                perfconfigs[j] for j in range(m) if x[j].varValue == 1
            ]

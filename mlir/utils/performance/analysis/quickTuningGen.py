        ...
        for arch in architectures:
            for instruction_type, datatypes in instruction_types_to_datatypes.items():
                for datatype in datatypes:
                    key = (arch, datatype)
                    if key not in instruction_type_cache:
                        instruction_type_cache[key] = self.get_instruction_type(arch, datatype)
        ...
        with open(file_path, 'w') as file:
            ...
            for instruction_type, datatypes in instruction_types_to_datatypes.items():
                ...
                for datatype in datatypes:
                    for op in ops:
                        for arch in architectures:
                            cached_instruction_type = instruction_type_cache.get((arch, datatype))
                            if cached_instruction_type == instruction_type:

...
def analyzeGemmFile(file, n):
    ...
    df[perfConfigParams] = df["PerfConfig"].str.replace("v2:", "").str.split(
        ",", expand=True)

    df["ArithmeticIntensity"] = df.apply(
        lambda row: calculateArithmeticIntensity(row["M"], row["N"], row["K"]),
        axis=1)
    df["MNPerWave"] = df.apply(lambda row:
                               (int(row["MPerWave"]) * int(row["NPerWave"])),
                               axis=1)
    df["Occupancy"] = df.apply(lambda row: calculateOccupancy(
        int(row["M"]), int(row["N"]), int(row["G"]), int(row["MPerBlock"]),
        int(row["NPerBlock"]), int(row["MNPerWave"]), minNumWaves),
                               axis=1)
    df["WorkImbalance"] = df.apply(lambda row: calculateWorkImbalance(
        int(row["M"]), int(row["N"]), int(row["G"]), int(row["MPerBlock"]),
        int(row["NPerBlock"]), int(row["MNPerWave"]), minNumWaves,
        int(row["splitKFactor"])),
                                   axis=1)
    ...
    df[[
        ...
        'TFlops', 'MPerBlock', 'NPerBlock', 'KPerBlock', 'MPerWave',
        'NPerWave', 'kPack', 'splitKFactor', 'forceUnroll', 'ThreadCopyMore',
...
def calculateWorkImbalance(M,
    ...
    return ((1 - (WorkImbalanceIntermedResult))
            if WorkImbalanceIntermedResult != 0 else 0)
...
if __name__ == "__main__":
    ...
    )  # numCUs (if data is not collected on the machine on which the script is executed)

...
def verifyKernelWithPerfConfig(perfConfig, config, paths: Paths, options: Options) -> float:
    ...
    os.system("rm -f "+perfRunner.BENCHMARKING_RESULT_FILE_NAME.replace(".csv", ".db"))
    ...
    profilerCommand = [perfRunner.ROCPROF, '--stats', '-o', perfRunner.BENCHMARKING_RESULT_FILE_NAME, paths.mlir_paths.cpu_runner_path] + mlirCpuRunnerArgs
...
Errors = {errs.decode('utf-8')}""", file=sys.stderr)
        ...
        return np.nan
    nanoSeconds = perfRunner.getNanoSeconds(perfRunner.BENCHMARKING_RESULT_FILE_NAME.replace(".csv", ".stats.csv"))
    return nanoSeconds
...
def main(args=None):
    ...
    perfRunner.BENCHMARKING_RESULT_FILE_NAME = f"results-{os.getpid()}.csv"
    ...
    with open(parsed_args.output, 'a') as outFile:
        if parsed_args.tflops:
            print("# arch\tnumCUs\ttestVector\tperfConfig\tTFlops", file=outFile)
        else:

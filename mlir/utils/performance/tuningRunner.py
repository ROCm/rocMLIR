...
def verifyKernelWithPerfConfig(perfConfig, config, paths: Paths, options: Options) -> float:
    ...
    profilerCommand = [perfRunner.ROCPROF, '-i', 'rocmlir_metrics.txt', '--kernel-trace', '--stats ', '-o', perfRunner.BENCHMARKING_STATS_FILE_NAME, '--', paths.mlir_paths.cpu_runner_path] + mlirCpuRunnerArgs

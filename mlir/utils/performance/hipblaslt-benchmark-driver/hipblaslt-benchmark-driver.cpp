  ...
  do {                                                                         \
    ...
    if (status != HIPBLAS_STATUS_SUCCESS) {                                    \
      fprintf(stderr, "HipBLASLt error %d at %s:%d in %s\n",                  \
              (int)status, __FILE__, __LINE__, #expr);                        \
      exit(1);                                                                 \
    }                                                                          \
  } while (0)
...
static void setBatchAttributes(hipblasLtMatrixLayout_t layout, int64_t batch_count,
                                int64_t stride) {
  HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatrixLayoutSetAttribute(
      layout, HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, &batch_count, sizeof(batch_count)));
  HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatrixLayoutSetAttribute(
      layout, HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &stride, sizeof(stride)));
}

int main(int argc, char **argv) {
  auto args = benchmark::parseCommandLine("hipblaslt-benchmark-driver", argc, argv);
  ...
  // To emulate row-major, we swap A and B matrices and compute B * A instead of A * B.
  const hipblasOperation_t trans_a = args.transposeB ? HIPBLAS_OP_T : HIPBLAS_OP_N;
  const hipblasOperation_t trans_b = args.transposeA ? HIPBLAS_OP_T : HIPBLAS_OP_N;
  const int64_t lda = args.transposeB ? k : n;
  const int64_t ldb = args.transposeA ? m : k;
  const int64_t ldc = n;
  const int64_t ldd = n;
  
  benchmark::DataType computeDataType = getComputeDataType(args.dataType, args.outDataType);
  ...
  const size_t aBytes = benchmark::getBytesPerElement(args.dataType) * strideA * batch_count;
  const size_t bBytes = benchmark::getBytesPerElement(args.dataType) * strideB * batch_count;
  const size_t cBytes = benchmark::getBytesPerElement(args.outDataType) * strideC * batch_count;
  ...
  HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatmulDescCreate(&matmul, computeType, scaleType));
  ...
  if (args.dataType == benchmark::DataType::F8) {
    ...
    HIP_ABORT_IF_FAIL(hipMemcpy(d_scale_a, &h_scale, sizeof(float), hipMemcpyHostToDevice));
    HIP_ABORT_IF_FAIL(hipMemcpy(d_scale_b, &h_scale, sizeof(float), hipMemcpyHostToDevice));
    
    hipblasLtMatmulMatrixScale_t scale_mode = HIPBLASLT_MATMUL_MATRIX_SCALE_SCALAR_32F;
    HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatmulDescSetAttribute(
        matmul, HIPBLASLT_MATMUL_DESC_A_SCALE_MODE, &scale_mode, sizeof(scale_mode)));
    HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatmulDescSetAttribute(
        matmul, HIPBLASLT_MATMUL_DESC_B_SCALE_MODE, &scale_mode, sizeof(scale_mode)));
    HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatmulDescSetAttribute(
        matmul, HIPBLASLT_MATMUL_DESC_A_SCALE_POINTER, &d_scale_a, sizeof(void*)));
    HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatmulDescSetAttribute(
        matmul, HIPBLASLT_MATMUL_DESC_B_SCALE_POINTER, &d_scale_b, sizeof(void*)));
  }

  hipblasLtMatrixLayout_t matA, matB, matC, matD;
  HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatrixLayoutCreate(&matA, inputType, n, k, lda));
  HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatrixLayoutCreate(&matB, inputType, k, m, ldb));
  HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatrixLayoutCreate(&matC, outputType, n, m, ldc));
  HIPBLASLT_ABORT_IF_FAIL(hipblasLtMatrixLayoutCreate(&matD, outputType, n, m, ldd));
  ...
  HIPBLASLT_ABORT_IF_FAIL(hipblaslt_ext::getAllAlgos(
      ...
      inputType, inputType, outputType, outputType, computeType, heuristicResults));
  ...
  for (size_t j = 0; j < heuristicResults.size(); j++) {
    ...
    if (hipblaslt_ext::matmulIsAlgoSupported(handle, matmul, alpha, matA, matB,
            beta, matC, matD, heuristicResults[j].algo, workspace_size) 
  ...
  for (int idx : validIdx) {
    maxWorkspaceSize = std::max(maxWorkspaceSize, heuristicResults[idx].workspaceSize);
  }
  ...
  for (size_t algoTestIdx = 0; algoTestIdx < validIdx.size(); ++algoTestIdx) {
    ...
    for (int i = 0, e = benchmarkRuns + warmupRuns; i < e; ++i) {
      ...
      hipblasStatus_t status = hipblasLtMatmul(
          ...
          &heuristicResults[algoIdx].algo, workspace, 
  ...
  const hipblasLtMatmulHeuristicResult_t bestResult = heuristicResults[bestAlgoIndex];
  ...
  if (workspace) HIP_ABORT_IF_FAIL(hipFree(workspace));
  if (d_scale_a) HIP_ABORT_IF_FAIL(hipFree(d_scale_a));
  if (d_scale_b) HIP_ABORT_IF_FAIL(hipFree(d_scale_b));

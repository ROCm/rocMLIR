...
// Padding_One-NEXT: rock.conv({{.*}}, {{.*}}, {{.*}}) features = {{.*}} {arch = "{{.*}}", dilations = [1 : i32, 1 : i32], filter_layout = ["g", "k", "c", "y", "x"], input_layout = ["ni", "gi", "ci", "hi", "wi"], numCU = {{.*}} : i32, output_layout = ["no", "go", "ko", "ho", "wo"], padding = [0 : i32, 0 : i32, 1 : i32, 2 : i32], strides = [1 : i32, 1 : i32]} : memref<1x256x32x1x1xf32>, memref<32x1x32x14x14xf32>, memref<32x1x256x14x17xf32>
...
// Padding_Two-NEXT: rock.conv({{.*}}, {{.*}}, {{.*}}) features = {{.*}} {arch = "{{.*}}", dilations = [1 : i32, 1 : i32], filter_layout = ["g", "k", "c", "y", "x"], input_layout = ["ni", "gi", "ci", "hi", "wi"], numCU = {{.*}} : i32, output_layout = ["no", "go", "ko", "ho", "wo"], padding = [3 : i32, 3 : i32, 1 : i32, 2 : i32], strides = [1 : i32, 1 : i32]} : memref<1x256x32x1x1xf32>, memref<32x1x32x14x14xf32>, memref<32x1x256x20x17xf32>

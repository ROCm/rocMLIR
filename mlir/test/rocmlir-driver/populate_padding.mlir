...
// Padding_One-NEXT: rock.conv2d({{.*}}, {{.*}}, {{.*}}) features = {{.*}}, convParams = {padding : [0, 0, 1, 2], stride : [1, 1], dilation : [1, 1]} {arch = "{{gfx[0-9a-z]+}}", filter_layout = ["g", "k", "c", "y", "x"], gemm_id = 0 : i32, input_layout = ["ni", "gi", "ci", "hi", "wi"], output_layout = ["no", "go", "ko", "ho", "wo"]} : memref<1x256x32x1x1xf32>, memref<32x1x32x14x14xf32>, memref<32x1x256x14x17xf32>
...
// Padding_Two-NEXT: rock.conv2d({{.*}}, {{.*}}, {{.*}}) features = {{.*}}, convParams = {padding : [3, 3, 1, 2], stride : [1, 1], dilation : [1, 1]} {arch = "{{gfx[0-9a-z]+}}", filter_layout = ["g", "k", "c", "y", "x"], gemm_id = 0 : i32, input_layout = ["ni", "gi", "ci", "hi", "wi"], output_layout = ["no", "go", "ko", "ho", "wo"]} : memref<1x256x32x1x1xf32>, memref<32x1x32x14x14xf32>, memref<32x1x256x20x17xf32>

// RUN: rocmlir-gen -p -prc | FileCheck %s --check-prefix=F32

// F32: func.func @main()
// F32:  [[RES:%.*]] = memref.cast {{.*}} : memref<{{.*}}> to memref<*xf32>
// F32-NEXT:    call @printMemrefF32([[RES]]) : (memref<*xf32>) -> ()

// RUN: rocmlir-gen -p -prc -t f16 | FileCheck %s --check-prefix=F16
...
// RUN: rocmlir-gen -p -prc -t bf16 | FileCheck %s --check-prefix=BF16
...
// RUN: rocmlir-gen -p -prc -t i8 | FileCheck %s --check-prefix=INT8

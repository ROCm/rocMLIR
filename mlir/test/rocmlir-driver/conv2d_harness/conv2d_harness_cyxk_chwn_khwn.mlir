...
// RUN: rocmlir-gen --arch %arch -p -fil_layout=gcyxk -in_layout=gchwn -out_layout=gkhwn %s | rocmlir-driver -c | mlir-cpu-runner -O2 --shared-libs=%linalg_test_lib_dir/%prefix_mlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/%prefix_conv-validation-wrappers%shlibext,%linalg_test_lib_dir/%prefix_mlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E
...
// LOWERING: module
// LOWERING: llvm.mlir.global internal constant @rock_conv2d_gcyxk_gchwn_gkhwn_0_module_gpubin_cst

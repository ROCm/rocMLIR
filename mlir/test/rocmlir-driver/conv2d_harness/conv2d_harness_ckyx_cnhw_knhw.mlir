// RUN: rocmlir-gen --arch %targetChip -p -fil_layout=gckyx -in_layout=gcnhw -out_layout=gknhw --host %s | FileCheck %s --check-prefix=HARNESS
// RUN: rocmlir-gen --arch %targetChip -p -fil_layout=gckyx -in_layout=gcnhw -out_layout=gknhw --host %s | rocmlir-driver -c | FileCheck %s --check-prefix=LOWERING
// RUN: rocmlir-gen --arch %targetChip -p -fil_layout=gckyx -in_layout=gcnhw -out_layout=gknhw --host %s | rocmlir-driver -c | mlir-rocm-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E
...
// LOWERING: module
// LOWERING: gpu.launch_func  @rock_conv2d_gckyx_gcnhw_gknhw_0_module::@rock_conv2d_gckyx_gcnhw_gknhw_0 blocks in (%{{.*}}, %{{.*}}, %{{.*}}) threads in (%{{.*}}, %{{.*}}, %{{.*}}) dynamic_shared_memory_size %{{.*}} args(%{{.*}} : memref<1x8x128x3x3xf32>, %{{.*}} : memref<1x8x128x32x32xf32>, %{{.*}} : memref<1x128x128x30x30xf32>)
...
// LOWERING: gpu.module @rock_conv2d_gckyx_gcnhw_gknhw_0_module
// LOWERING: gpu.func @rock_conv2d_gckyx_gcnhw_gknhw_0
// TBD. Add more verifying logic.

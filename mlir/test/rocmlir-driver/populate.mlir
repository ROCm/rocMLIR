// RUN: rocmlir-gen --arch %targetChip -p -mfma=off -atomic_add=off -dot=on | FileCheck %s --check-prefix=F32
// RUN: rocmlir-gen --arch %targetChip -p -t f16 -mfma=off -atomic_add=off -dot=on | FileCheck %s --check-prefix=F16
// RUN: rocmlir-gen --arch %targetChip -p -t bf16 -mfma=off -atomic_add=off -dot=on | FileCheck %s --check-prefix=BF16
// RUN: rocmlir-gen --arch %targetChip -p -t i8 -mfma=off -atomic_add=off -dot=on | FileCheck %s --check-prefix=INT8

...
// RUN: rocmlir-gen --operation gemm -t f32 --arch gfx90a -g 1 -m 128 -k 1024 -n 256 --perf_config "v4:64,64,16,32,32,16,4,4,1,2,0,0,1,1" | rocmlir-driver --kernel-pipeline=applicability 2>&1 | FileCheck %s --check-prefix=VALID-SPLITK
// VALID-SPLITK: rock.threadwise_gemm_accel
// VALID-SPLITK-NOT: error

// Test splitK=1 (no split) config passes applicability pipeline
// RUN: rocmlir-gen --operation gemm -t f32 --arch gfx90a -g 1 -m 128 -k 1024 -n 256 --perf_config "v4:64,64,16,32,32,16,4,1,1,2,0,0,1,1" | rocmlir-driver --kernel-pipeline=applicability 2>&1 | FileCheck %s --check-prefix=VALID-NOSPLIT
...
// RUN: rocmlir-gen --arch gfx90a --operation attention -t f16 -seq_len_q 256 -seq_len_k 256 -head_dim_qk 64 -head_dim_v 64 -g 1 -perf_config "attn:v3:32,32,32,32,32,32,16,8,1,1,2,0,1" | rocmlir-driver --kernel-pipeline=applicability 2>&1 | FileCheck %s --check-prefix=ATTN-VALID

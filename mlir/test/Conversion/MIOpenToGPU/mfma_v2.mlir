...
module {
  ...
  func @mfma_bf16(%a : vector<2xi16>, %b : vector<2xi16>, %c : vector<32xf32>) attributes {kernel = 0 : i32} {
    ...
    // CHECK: %[[D0:.*]] = gpu.mfma(%{{.*}}, %{{.*}}, %{{.*}}) {imm = [1, 0, 0], instr = "mfma_f32_32x32x2bf16"} : vector<2xi16>, vector<32xf32>
    %d1 = miopen.mfma_v2(%a, %b, %d0) { instr = "mfma_f32_32x32x2bf16", imm = [1, 0, 0]}: vector<2xi16>, vector<32xf32>
    // CHECK: %[[D1:.*]] = gpu.mfma(%{{.*}}, %{{.*}}, %[[D0]]) {imm = [1, 0, 0], instr = "mfma_f32_32x32x2bf16"} : vector<2xi16>, vector<32xf32>

...
func.func @mlir_reshape_transpose_reshape_convolution(%arg0: tensor<1x116x28x28xf32>, %arg1: tensor<116x1x3x3xf32>) -> tensor<1x116x14x14xf32> attributes {arch = "gfx1100", kernel = "mixr"} {
  ...
  // CHECK: [[GC_TR:%.+]] = tosa.transpose [[EXPANDED]], %{{.*}} : (tensor<1x2x58x28x28xf32>, tensor<5xi32>) -> tensor<1x58x2x28x28xf32>
  ...
  %0 = "tosa.const"() <{value = dense<[0, 2, 1, 3, 4]> : tensor<5xi32>}> : () -> tensor<5xi32>
  %1 = tosa.transpose %expanded, %0 : (tensor<1x2x58x28x28xf32>, tensor<5xi32>) -> tensor<1x58x2x28x28xf32>

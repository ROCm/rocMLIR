...
// Test tosa.matmul_t_block_scaled lowering to rock.gemm with scales
...
// CHECK-LABEL: mlir_matmul_t_block_scaled_basic
// CHECK: rock.transform
// CHECK: rock.gemm %{{.*}} = %{{.*}} scaled by %{{.*}} * tr %{{.*}} scaled by tr %{{.*}}

func.func @mlir_matmul_t_block_scaled_basic(%a_data: tensor<1x128x256xf4E2M1FN>, 
...
// Test tosa.matmul_t_block_scaled with batched inputs

// CHECK-LABEL: mlir_matmul_t_block_scaled_batched
// CHECK: rock.transform
// CHECK: rock.gemm %{{.*}} = %{{.*}} scaled by %{{.*}} * tr %{{.*}} scaled by tr %{{.*}}

func.func @mlir_matmul_t_block_scaled_batched(%a_data: tensor<4x128x256xf4E2M1FN>, 
...
// Test tosa.matmul_t_block_scaled with larger K dimension

// CHECK-LABEL: mlir_matmul_t_block_scaled_large_k
// CHECK: rock.transform
// CHECK: rock.gemm %{{.*}} = %{{.*}} scaled by %{{.*}} * tr %{{.*}} scaled by tr %{{.*}}

func.func @mlir_matmul_t_block_scaled_large_k(%a_data: tensor<1x64x512xf4E2M1FN>, 
                                               %a_scale: tensor<1x64x16xf8E8M0FNU>,
                                               %b_data: tensor<1x256x512xf4E2M1FN>, 
                                               %b_scale: tensor<1x256x16xf8E8M0FNU>) 
                                               -> tensor<1x64x256xf32> attributes {kernel, arch = "##TOKEN_ARCH##"} {
  %result = tosa.matmul_t_block_scaled %a_data, %a_scale, %b_data, %b_scale {block_size = #tosa.block_size<BLOCK_SIZE_32>} 

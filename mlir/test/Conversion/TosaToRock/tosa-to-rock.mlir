...
// CHECK: %[[convRes:.*]] = rock.conv2d(%{{.*}}, %{{.*}}, %{{.*}}) features = {{none|xdlops}} {arch = {{.*}}, dilations = [1 : i32, 1 : i32], filter_layout = ["k", "y", "x", "c", "g"], has_relayout_do_not_unfold, input_layout = ["ni", "hi", "wi", "ci", "gi"], output_layout = ["no", "ho", "wo", "ko", "go"], padding = [0 : i32, 0 : i32, 0 : i32, 0 : i32], strides = [1 : i32, 1 : i32]} : tensor<128x8x3x3x1xf32>, tensor<128x8x32x32x1xf32>, tensor<128x128x30x30x1xf32>

...
func.func @mlir_bwd_data_conv(%arg0: tensor<150xf32>, %arg1: tensor<54xf32>) -> (tensor<2210xf32>, tensor<2210xf32>) attributes {rock.arch = "gfx950", rock.kernel} {
    %cst = arith.constant dense<0.000000e+00> : tensor<1x2x1x9x19x25xf32>
    %expanded = tensor.expand_shape %arg0 [[0, 1, 2, 3, 4, 5]] output_shape [1, 2, 1, 3, 5, 5] : tensor<150xf32> into tensor<1x2x1x3x5x5xf32>
    %expanded_0 = tensor.expand_shape %arg1 [[0, 1, 2, 3, 4, 5]] output_shape [2, 1, 1, 3, 3, 3] : tensor<54xf32> into tensor<2x1x1x3x3x3xf32>
    // expected-error @+2 {{backward convolution only supports function with a single result}}
    ...
    %extracted_slice = tensor.extract_slice %collapsed[0, 0, 2, 3, 4] [1, 2, 5, 13, 17] [1, 1, 1, 1, 1] : tensor<1x2x9x19x25xf32> to tensor<1x2x5x13x17xf32>
    %collapsed_1 = tensor.collapse_shape %extracted_slice [[0, 1, 2, 3, 4]] : tensor<1x2x5x13x17xf32> into tensor<2210xf32>
    return %collapsed_1, %collapsed_1 : tensor<2210xf32>, tensor<2210xf32>
}
...
func.func @mlir_bwd_data_conv(%arg0: tensor<150xf32>, %arg1: tensor<54xf32>) -> tensor<2210xf32> attributes {rock.arch = "gfx950", rock.kernel} {
    ...
    %expanded = tensor.expand_shape %arg0 [[0, 1, 2, 3, 4, 5]] output_shape [1, 2, 1, 3, 5, 5] : tensor<150xf32> into tensor<1x2x1x3x5x5xf32>
    %expanded_0 = tensor.expand_shape %arg1 [[0, 1, 2, 3, 4, 5]] output_shape [2, 1, 1, 3, 3, 3] : tensor<54xf32> into tensor<2x1x1x3x3x3xf32>
    // note that there is a linalg.add after the tensor.collapsed_shape that triggers this warning
    // expected-error @+2 {{invalid padding code structure}}
    // expected-error @+1 {{failed to legalize operation}}
    %0 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "parallel", "parallel", "parallel", "reduction", "reduction", "reduction", "reduction"]} ins(%expanded, %expanded_0 : tensor<1x2x1x3x5x5xf32>, tensor<2x1x1x3x3x3xf32>) outs(%cst : tensor<1x2x1x9x19x25xf32>) attrs =  {conv_op = #rock<LinalgConvType convbwd3d_ngchwd_gckhwd>, dilation = [2, 3, 4], group = 2 : i64, pad = [2, 3, 4, 2, 3, 4], stride = [2, 3, 4]} {
    ^bb0(%in: f32, %in_2: f32, %out: f32):
        %1 = arith.mulf %in, %in_2 : f32
    ...
    %collapsed = tensor.collapse_shape %0 [[0], [1, 2], [3], [4], [5]] : tensor<1x2x1x9x19x25xf32> into tensor<1x2x9x19x25xf32>
    %placeholder = tensor.empty() : tensor<1x2x9x19x25xf32>
    %add = linalg.add ins(%collapsed, %placeholder : tensor<1x2x9x19x25xf32>, tensor<1x2x9x19x25xf32>) outs(%placeholder : tensor<1x2x9x19x25xf32>) -> tensor<1x2x9x19x25xf32>
    %extracted_slice = tensor.extract_slice %collapsed[0, 0, 2, 3, 4] [1, 2, 5, 13, 17] [1, 1, 1, 1, 1] : tensor<1x2x9x19x25xf32> to tensor<1x2x5x13x17xf32>
    %collapsed_1 = tensor.collapse_shape %extracted_slice [[0, 1, 2, 3, 4]] : tensor<1x2x5x13x17xf32> into tensor<2210xf32>
    return %collapsed_1 : tensor<2210xf32>
}
...
#map = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d4, d2)>
#map1 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d1, d4, d3, d5)>
#map2 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d3, d2 + d5)>

func.func @mlir_bwd_data_conv(%arg0: tensor<8xf32>, %arg1: tensor<18xf32>) -> tensor<12xf32> attributes {rock.arch = "gfx950", rock.kernel} {
    %cst = arith.constant dense<0.000000e+00> : tensor<1x1x3x6xf32>
    %expanded = tensor.expand_shape %arg0 [[0, 1, 2, 3]] output_shape [1, 1, 2, 4] : tensor<8xf32> into tensor<1x1x2x4xf32>
    %expanded_0 = tensor.expand_shape %arg1 [[0, 1, 2, 3]] output_shape [1, 2, 3, 3] : tensor<18xf32> into tensor<1x2x3x3xf32>
    // expected-error @+2 {{conv1d backward conv is not supported for now}}
    // expected-error @+1 {{failed to legalize operation}}
    %0 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "parallel", "reduction", "reduction"]} ins(%expanded, %expanded_0 : tensor<1x1x2x4xf32>, tensor<1x2x3x3xf32>) outs(%cst : tensor<1x1x3x6xf32>) attrs =  {conv_op = #rock<LinalgConvType convbwd1d_ngch_gckh>, dilation = [1], group = 1 : i64, pad = [1, 1], stride = [1]} {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
            %1 = arith.mulf %in, %in_2 : f32
            %2 = arith.addf %out, %1 : f32
            linalg.yield %2 : f32
    } -> tensor<1x1x3x6xf32>
    %collapsed = tensor.collapse_shape %0 [[0], [1, 2], [3]] : tensor<1x1x3x6xf32> into tensor<1x3x6xf32>
    %extracted_slice = tensor.extract_slice %collapsed[0, 0, 1] [1, 3, 4] [1, 1, 1] : tensor<1x3x6xf32> to tensor<1x3x4xf32>
    %collapsed_1 = tensor.collapse_shape %extracted_slice [[0, 1, 2]] : tensor<1x3x4xf32> into tensor<12xf32>
    return %collapsed_1 : tensor<12xf32>
}

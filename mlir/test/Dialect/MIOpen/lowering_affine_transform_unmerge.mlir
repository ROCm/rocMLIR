...
module  {
  func @miopen_unmerge_test(%arg0: memref<1x144x128xf32>) {
    ...
    %0 = miopen.transform(%arg0) {gridwise_gemm_argument_position = 0 : i32, layout = [{upper_layer_dimensions = [0 : i32], upper_layer_names = ["gemmG"],
         ...
         parameters = [18 : i32, 8 : i32], lower_layer_names = ["gemmKtotal"], transformation = "UnMerge"},{upper_layer_dimensions = [2 : i32], upper_layer_names = ["gemmM"], lower_layer_dimensions = [2 : i32], lower_layer_names = ["gemmM"], transformation = "PassThrough"}],

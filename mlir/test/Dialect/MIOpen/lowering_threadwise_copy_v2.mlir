...
#map0 = affine_map<(d0, d1, d2) -> (d0 * 8 + d1 * 4 + d2)>
#map6 = affine_map<(d0, d1, d2, d3, d4) -> (d1 * 4 + d3)>
#map7 = affine_map<(d0, d1, d2, d3, d4) -> (d0, d1 * 8 + d2 * 4 + d3, d4)>
#map8 = affine_map<(d0, d1, d2) -> (d2 floordiv 196, d0, d1, (d2 mod 196) floordiv 14, (d2 mod 196) mod 14)>
...
func @miopen_threadwise_copy_v2(%source_offset : i32,
  ...
  // CHECK-NOT: scf.for
  miopen.threadwise_copy_v2(%source, %dest5D, %source_offset,
                            %c0_i32, %c0_i32, %c0_i32, %c0_i32, %c0_i32,
                            %c0_i32, %c0_i32, %c0_i32, %c0_i32, %c0_i32) {
    bound = [1 : i32, 4 : i32, 1 : i32, 4 : i32, 1 : i32], 
    coord_transforms = [ 
      {
        operand = 0 : i32, transforms = [#map6],
        metadata = [
          {layout = [
             {lower_layer_dimensions = [0 : i32],
              lower_layer_names = ["raw"],
              ...
              upper_layer_names = ["g", "m0", "m1", "m2", "n"]}
           ],
           lower_layer_bounds = [32 : i32], // FIXME. CHECK THIS.
           lower_layer_layout = ["raw"],
           upper_layer_bounds = [1 : i32, 4 : i32, 1 : i32, 4 : i32, 1 : i32],
           upper_layer_layout = ["g", "m0", "m1", "m2", "n"]}
         ]
      },
      {
        operand = 1 : i32, transforms = [#map7, #map8],
  ...
  } : vector<32xf32>, memref<128x1x1024x14x14xf32>

...
func.func @rock_accel_gemm_fma(%dummy: memref<1x2xvector<32xf16>>) attributes {block_size = 64 : i32} {
  ...
  rock.threadwise_accel_gemm %76 += %74 * %75 at[%c0, %c0, %c0] features =  dot|atomic_add|atomic_add_f16 {arch = "amdgcn-amd-amdhsa:gfx942:sramecc+:xnack-", params = #rock.fma_gemm_params<kpackPerBlock = 2, mPerBlock = 1, nPerBlock = 1, kpack = 32, mPerWave = 1, nPerWave = 1, splitKFactor = 1, scheduleVersion = 1, outputSwizzle = 2, forceUnroll = true>} : memref<1x1xvector<1xf32>, #gpu.address_space<private>> += memref<1x2xvector<32xf16>, #gpu.address_space<private>> * memref<1x2xvector<32xf16>, #gpu.address_space<private>>
  return
}

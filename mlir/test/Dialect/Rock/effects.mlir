...
func.func @rock_blockwise_gemm_accel(%bufferA : memref<4xvector<8xf8E4M3FN>, #gpu.address_space<private>>,
  ...
  rock.blockwise_gemm_accel %matrixC += %bufferA * %bufferB features = mfma {
    ...
    blockSize = 256 : i32,
    matrixParamsA = #rock.blockwise_matrix_params<elementType = f8E4M3FN, elementTypeLoad = f8E4M3FN, rotateDWithK = false, swapThreadIterSubDims = false, LDSLayout = KxDxkpack, directToLDS = false, splitKAcrossThreadsFirst = false, accelLayout = false, g = 1, d = 64, inDPerThread = 2>, 
    matrixParamsB = #rock.blockwise_matrix_params<elementType = f8E5M2, elementTypeLoad = f8E5M2, rotateDWithK = false, swapThreadIterSubDims = false, LDSLayout = KxDxkpack, directToLDS = false, splitKAcrossThreadsFirst = false, accelLayout = false, g = 1, d = 256, inDPerThread = 2>,
    params = #rock.mfma_gemm_params<
      kpackPerBlock = 8,
      mPerBlock = 128,
      nPerBlock = 128,
...
func.func @rock_blockwise_gemm_accel_lds(%matrixA : memref<1024xvector<8xf8E4M3FN>, #gpu.address_space<workgroup>>,
...
func.func @rock_blockwise_gemm_accel_two_results(%matrixA : memref<256xvector<2xf4E2M1FN>, #gpu.address_space<workgroup>>, 
...
func.func @loadtile_doublebuffer(%arg0: memref<1x384x64xf32>, %lds: memref<4096xi8, #gpu.address_space<workgroup>>, %reg: memref<16xf32, #gpu.address_space<private>>) attributes {arch = "##TOKEN_ARCH##"} {
  ...
  affine.for %arg1 = 0 to 2 {
    ...
    // expected-remark @below {{found an instance of 'read' on op operand 1, on resource '<Default>'}}
    // expected-remark @below {{found an instance of 'write' on op operand 2, on resource '<Default>'}}
    rock.blockwise_load_tile %0[%arg1, %c0, %c0, %c0, %c0] LDS -> %lds -> %reg {elementType = f32, elementLoadType = f32, matrixParamsA = #rock.blockwise_matrix_params<elementType = f32, elementTypeLoad = f32, rotateDWithK = false, swapThreadIterSubDims = false, LDSLayout = KxDxkpack, directToLDS = false, splitKAcrossThreadsFirst = false, accelLayout = false, g = 1, d = 384, inDPerThread = 1>, matrixParamsB = #rock.blockwise_matrix_params<elementType = f32, elementTypeLoad = f32, rotateDWithK = false, swapThreadIterSubDims = false, LDSLayout = KxDxkpack, directToLDS = false, splitKAcrossThreadsFirst = false, accelLayout = false, g = 1, d = 384, inDPerThread = 4>, blockSize = 64 : i32, loadType = #rock<GemmLoadTileType DoubleBuffer>, params = #rock.mfma_gemm_params<kpackPerBlock = 32, mPerBlock = 32, nPerBlock = 32, kpack = 1, mPerWave = 32, nPerWave = 32, mnPerXdl = 32, splitKFactor = 1, scheduleVersion = 2, outputSwizzle = 2, wavesPerEU = 0, gridGroupSize = 0, forceUnroll = true>} : memref<1x64x384xf32> LDS -> memref<4096xi8, #gpu.address_space<workgroup>> -> memref<16xf32, #gpu.address_space<private>>
  }
  return
}
...
func.func @loadtile_doublebuffer_directtolds(%arg0: memref<1x384x64xf32>, %lds: memref<4096xi8, #gpu.address_space<workgroup>>, %reg: memref<16xf32, #gpu.address_space<private>>) attributes {arch = "##TOKEN_ARCH##"} {
  ...
  affine.for %arg1 = 0 to 2 {
    ...
    // expected-remark @below {{found an instance of 'read' on op operand 1, on resource '<Default>'}}
    // expected-remark @below {{found an instance of 'write' on op operand 2, on resource '<Default>'}}
    rock.blockwise_load_tile %0[%arg1, %c0, %c0, %c0, %c0] LDS -> %lds -> %reg {elementType = f32, elementLoadType = f32, matrixParamsA = #rock.blockwise_matrix_params<elementType = f32, elementTypeLoad = f32, rotateDWithK = false, swapThreadIterSubDims = false, LDSLayout = KxDxkpack, directToLDS = true, splitKAcrossThreadsFirst = false, accelLayout = false, g = 1, d = 384, inDPerThread = 1>, matrixParamsB = #rock.blockwise_matrix_params<elementType = f32, elementTypeLoad = f32, rotateDWithK = false, swapThreadIterSubDims = false, LDSLayout = KxDxkpack, directToLDS = true, splitKAcrossThreadsFirst = false, accelLayout = false, g = 1, d = 384, inDPerThread = 4>, blockSize = 64 : i32, loadType = #rock<GemmLoadTileType DirectToLDSDoubleBuffer>, params = #rock.mfma_gemm_params<kpackPerBlock = 32, mPerBlock = 32, nPerBlock = 32, kpack = 1, mPerWave = 32, nPerWave = 32, mnPerXdl = 32, splitKFactor = 1, scheduleVersion = 4, outputSwizzle = 2, wavesPerEU = 0, gridGroupSize = 0, forceUnroll = true>} : memref<1x64x384xf32> LDS -> memref<4096xi8, #gpu.address_space<workgroup>> -> memref<16xf32, #gpu.address_space<private>>
  }
  return
}
...
func.func @loadtile_default_directtolds(%arg0: memref<1x384x64xf32>, %lds: memref<4096xi8, #gpu.address_space<workgroup>>) attributes {arch = "##TOKEN_ARCH##"} {
  ...
  affine.for %arg1 = 0 to 2 {
    // expected-remark @below {{found an instance of 'read' on op operand 0, on resource '<Default>'}}
    // expected-remark @below {{found an instance of 'write' on op operand 1, on resource '<Default>'}}
    rock.blockwise_load_tile %0[%arg1, %c0, %c0, %c0, %c0] LDS -> %lds {elementType = f32, elementLoadType = f32, matrixParamsA = #rock.blockwise_matrix_params<elementType = f32, elementTypeLoad = f32, rotateDWithK = false, swapThreadIterSubDims = false, LDSLayout = KxDxkpack, directToLDS = true, splitKAcrossThreadsFirst = false, accelLayout = false, g = 1, d = 384, inDPerThread = 1>, matrixParamsB = #rock.blockwise_matrix_params<elementType = f32, elementTypeLoad = f32, rotateDWithK = false, swapThreadIterSubDims = false, LDSLayout = KxDxkpack, directToLDS = true, splitKAcrossThreadsFirst = false, accelLayout = false, g = 1, d = 384, inDPerThread = 4>, blockSize = 64 : i32, loadType = #rock<GemmLoadTileType DirectToLDSDefault>, params = #rock.mfma_gemm_params<kpackPerBlock = 32, mPerBlock = 32, nPerBlock = 32, kpack = 1, mPerWave = 32, nPerWave = 32, mnPerXdl = 32, splitKFactor = 1, scheduleVersion = 3, outputSwizzle = 2, wavesPerEU = 0, gridGroupSize = 0, forceUnroll = true>} : memref<1x64x384xf32> LDS -> memref<4096xi8, #gpu.address_space<workgroup>>
  }
  return
}

...
func.func @rock_blockwise_reducesum_nr_threads_lt_blocksize(%input : memref<1x20x32xf32>,  %output : memref<1x1x32xf32>) attributes{rock.arch = "##TOKEN_ARCH##", block_size = 20 : i32, grid_size = 8 : i32, rock.kernel} {
  ...
  %ws_lds = memref.view %ws_lds_bytes[%c0][] : memref<320xi8, #gpu.address_space<workgroup>> to memref<80xf32, #gpu.address_space<workgroup>>
  ...
  rock.blockwise_broadcast_reduce sum [#transform_map5][#transform_map5_tid][#transform_map5_iter]%input_reg into %output_reg using %ws_lds {axis = 1 : index, blockSize = 20 : i32} : memref<4xf32, #gpu.address_space<private>> using memref<80xf32, #gpu.address_space<workgroup>> into memref<4xf32, #gpu.address_space<private>>

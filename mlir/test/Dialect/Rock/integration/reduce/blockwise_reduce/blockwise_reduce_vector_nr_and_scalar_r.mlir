...
#transform_map21 = #rock.transform_map<#map12 by [<Merge{1, 2, 32} ["tid"] at [0] -> ["1ave", "m_tid", "n_tid"] at [0, 1, 2]>, <Merge{1, 1, 4, 4} ["item"] at [1] -> ["i", "j", "vec_group", "vec_item"] at [3, 4, 5, 6]>] bounds = [64, 16] -> [1, 2, 32, 1, 1, 4, 4]>
#transform_map22 = #rock.transform_map<#map13 by [<Merge{1, 1} ["1ave"] at [0] -> ["1ave_m", "1ave_n"] at [0, 1]>, <PassThrough ["m_tid", "n_tid"] at [1, 2] -> ["m_tid", "n_tid"] at [2, 3]>, <Merge{1, 1} ["i"] at [3] -> ["m_i", "n_i"] at [4, 5]>, <Merge{1, 1} ["j"] at [4] -> ["blk_row", "blk_col"] at [6, 7]>, <PassThrough ["vec_group", "vec_item"] at [5, 6] -> ["vec_group", "vec_item"] at [8, 9]>] bounds = [1, 2, 32, 1, 1, 4, 4] -> [1, 1, 2, 32, 1, 1, 1, 1, 4, 4]>
#transform_map23 = #rock.transform_map<#map14 by [<Unmerge{1, 1, 1, 4, 2, 4} ["m_i", "1ave_m", "blk_row", "vec_group", "m_tid", "vec_item"] at [4, 0, 6, 8, 2, 9] -> ["gemmM"] at [0]>, <Unmerge{1, 1, 1, 32} ["n_i", "1ave_n", "blk_col", "n_tid"] at [5, 1, 7, 3] -> ["gemmN"] at [1]>] bounds = [1, 1, 2, 32, 1, 1, 1, 1, 4, 4] -> [32, 32]>

#transform_map21_tid = #rock.transform_map<#map12_tid by [<Merge{1, 2, 32} ["tid"] at [0] -> ["1ave", "m_tid", "n_tid"] at [0, 1, 2]>] bounds = [64] -> [1, 2, 32]>
#transform_map22_tid = #rock.transform_map<#map13_tid by [<Merge{1, 1} ["1ave"] at [0] -> ["1ave_m", "1ave_n"] at [0, 1]>, <PassThrough ["m_tid", "n_tid"] at [1, 2] -> ["m_tid", "n_tid"] at [2, 3]>] bounds = [1, 2, 32] -> [1, 1, 2, 32]>
#transform_map23_tid = #rock.transform_map<#map14_tid by [<Unmerge{1, 2} ["1ave_m", "m_tid"] at [0, 2] -> ["gemmM"] at [0]>, <Unmerge{1, 32} ["1ave_n","n_tid"] at [1, 3] -> ["gemmN"] at [1]>] bounds = [1, 1, 2, 32] -> [2, 32]>
...
func.func @rock_blockwise_vector_nr_and_scalar_r(%input : memref<1x64x384xf32>,  %output : memref<1x64x384xf32>) attributes{arch = "##TOKEN_ARCH##", block_size = 64 : i32, grid_size = 24 : i32, kernel} {
  ...
  rock.dealloc %ws_lds_bytes : memref<256xi8, #gpu.address_space<workgroup>>

  rock.threadwise_write_all {forceUnroll, useIndexDiffs} %output_reg -> [#transform_map1, #transform_map2, #transform_map3](%output)[%c0, %m_block_id, %n_block_id, %tid] by set : memref<16xf32, #gpu.address_space<private>> -> memref<1x64x384xf32>
  return
}

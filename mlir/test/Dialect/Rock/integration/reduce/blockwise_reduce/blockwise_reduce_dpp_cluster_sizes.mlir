// RUN: rocmlir-gen -ph -print-results -rand none -fut test_dpp_cluster2 - < %s | sed s/##TOKEN_ARCH##/%arch/g | rocmlir-driver -arch %arch -c | mlir-runner -O2 --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=CHECK_C2
...
#c2_map0 = #rock.transform_map<affine_map<(d0, d1) -> (0, d1, d0)> by [<Merge{1, 1} ["nr"] at [0] -> ["1", "z"] at [0, 2]>, <PassThrough ["r"] at [1] -> ["0"] at [1]>] bounds = [1, 2] -> [1, 2, 1]>
#c2_map1 = #rock.transform_map<affine_map<(d0, d1, d2) -> (d0 * 1 + d1, d2)> by [<Unmerge{1, 1} ["bid", "nr_per_bid"] at [0, 1] -> ["nr"] at [0]>, <PassThrough ["r"] at [2] -> ["r"] at [1]>] bounds = [1, 1, 2] -> [1, 2]>
#c2_map2 = #rock.transform_map<affine_map<(d0, d1, d2) -> (d0, d2, d1)> by [<PassThrough ["bid"] at [0] -> ["bid"] at [0]>, <PassThrough ["tid"] at [1] -> ["r"] at [2]>, <PassThrough ["iter"] at [2] -> ["nr_per_bid"] at [1]>] bounds = [1, 2, 1] -> [1, 1, 2]>
#c2_map3 = #rock.transform_map<affine_map<(d0, d1) -> (0, d1, d0)> by [<Merge{1, 1} ["nr"] at [0] -> ["1", "z"] at [0, 2]>, <PassThrough ["r"] at [1] -> ["0"] at [1]>] bounds = [1, 1] -> [1, 1, 1]>
#c2_map4 = #rock.transform_map<affine_map<(d0, d1, d2) -> (d0 * 1 + d2, d1)> by [<Unmerge{1, 1} ["bid", "iter"] at [0, 2] -> ["nr"] at [0]>, <PassThrough ["tid"] at [1] -> ["r"] at [1]>] bounds = [1, 1, 1] -> [1, 1]>
#c2_map5 = #rock.transform_map<affine_map<(d0, d1) -> (d1, d0)> by [<PassThrough ["tid"] at [0] -> ["r"] at [1]>, <PassThrough ["iter"] at [1] -> ["nr_per_bid"] at [0]>] bounds = [2, 1] -> [1, 2]>
#c2_map5_tid = #rock.transform_map<affine_map<(d0) -> (0, d0)> by [<Merge{1, 2} ["tid"] at [0] -> ["nr_per_bid", "r"] at [0, 1]>] bounds = [2] -> [1, 2]>
#c2_map5_iter = #rock.transform_map<affine_map<(d0) -> (d0, 0)> by [<Merge{1, 1} ["iter"] at [0] -> ["nr_per_bid", "r"] at [0, 1]>] bounds = [1] -> [1, 1]>

func.func @test_dpp_cluster2(%input : memref<1x2x1xf32>, %output : memref<1x1x1xf32>) attributes{arch = "##TOKEN_ARCH##", block_size = 2 : i32, grid_size = 1 : i32, kernel} {
  %input_reg = rock.alloc() : memref<1xf32, #gpu.address_space<private>>
  %output_reg = rock.alloc() : memref<1xf32, #gpu.address_space<private>>
  %ws_lds_bytes = rock.alloc() : memref<8xi8, #gpu.address_space<workgroup>>
  %c0 = arith.constant 0 : index
  %ws_lds = memref.view %ws_lds_bytes[%c0][] : memref<8xi8, #gpu.address_space<workgroup>> to memref<2xf32, #gpu.address_space<workgroup>>
  %bid = rock.workgroup_id : index
  %tid = rock.workitem_id : index
  rock.threadwise_read_into {forceUnroll, useIndexDiffs}
    [#c2_map2, #c2_map1, #c2_map0](%input)[%bid, %tid] -> %input_reg : memref<1x2x1xf32> -> memref<1xf32, #gpu.address_space<private>>
  rock.blockwise_broadcast_reduce sum [#c2_map5][#c2_map5_tid][#c2_map5_iter]%input_reg into %output_reg using %ws_lds {axis = 1 : index, blockSize = 2 : i32} : memref<1xf32, #gpu.address_space<private>> using memref<2xf32, #gpu.address_space<workgroup>> into memref<1xf32, #gpu.address_space<private>>
  rock.threadwise_write_all {forceUnroll, useIndexDiffs} %output_reg -> [#c2_map4, #c2_map3](%output)[%bid, %tid] by set : memref<1xf32, #gpu.address_space<private>> -> memref<1x1x1xf32>
  return
}

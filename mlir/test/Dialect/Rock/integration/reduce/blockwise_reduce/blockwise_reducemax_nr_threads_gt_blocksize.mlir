...
#transform_map0 = #rock.transform_map<affine_map<(d0, d1) -> (0, d1, d0)> by [<Merge{1, 160} ["nr"] at [0] -> ["1", "z"] at [0, 2]>, <PassThrough ["r"] at [1] -> ["0"] at [1]>] bounds = [160, 20] -> [1, 20, 160]>
...
#transform_map3 = #rock.transform_map<affine_map<(d0, d1) -> (0, d1, d0)> by [<Merge{1, 160} ["nr"] at [0] -> ["1", "z"] at [0, 2]>, <PassThrough ["r"] at [1] -> ["0"] at [1]>] bounds = [160, 1] -> [1, 1, 160]>
...
func.func @rock_blockwise_reducesum_nr_threads_gt_blocksize(%input : memref<1x20x160xf32>,  %output : memref<1x1x160xf32>) attributes{arch = "##TOKEN_ARCH##", block_size = 20 : i32, grid_size = 8 : i32, kernel} {
  ...
  rock.threadwise_write_all {forceUnroll, useIndexDiffs} %output_reg -> [#transform_map4, #transform_map3](%output)[%bid, %tid] by set : memref<20xf32, #gpu.address_space<private>> -> memref<1x1x160xf32>
  return
}

...
func.func @rock_blockwise_reducesum_nr_threads_gt_blocksize(%input : memref<1x20x160xf32>,  %output : memref<1x1x160xf32>) attributes{arch = "", block_size = 20 : i32, grid_size = 8 : i32, kernel} {
  ...
  rock.threadwise_read_into {forceUnroll, useIndexDiffs}
    [#transform_map2, #transform_map1, #transform_map0](%input) -> %input_reg : memref<1x20x160xf32> ->  memref<20xf32, #gpu.address_space<private>>
  rock.blockwise_reduce max [#transform_map2]%input_reg into %output_reg using %ws_lds {axis = 2 : index, gridSize = 8 : i32, blockSize = 20 : i32} : memref<20xf32, #gpu.address_space<private>> using memref<400xf32, #gpu.address_space<workgroup>> into memref<20xf32, #gpu.address_space<private>>
  rock.threadwise_write_all features = none {forceUnroll, useIndexDiffs} %output_reg -> [#transform_map4, #transform_map3](%output) by set : memref<20xf32, #gpu.address_space<private>> -> memref<1x1x160xf32>
  return
}

...
func.func @rock_threadwise_memcopy_simple(%input : memref<1x20x32xf32>,  %output : memref<1x20x32xf32>) attributes{arch = "", block_size = 20 : i32, grid_size = 8 : i32, kernel} {
  ...
  rock.threadwise_write_all features = none {forceUnroll, useIndexDiffs} %input_reg -> [#transform_map3](%ws_lds) by set : memref<4xf32, #gpu.address_space<private>> -> memref<4x20xf32, #gpu.address_space<workgroup>>
  rock.threadwise_read_into {forceUnroll, useIndexDiffs}
    [#transform_map3](%ws_lds) -> %output_reg : memref<4x20xf32, #gpu.address_space<workgroup>> ->  memref<4xf32, #gpu.address_space<private>>
  rock.threadwise_write_all features = none {forceUnroll, useIndexDiffs} %output_reg -> [#transform_map2, #transform_map1, #transform_map0](%output) by set : memref<4xf32, #gpu.address_space<private>> -> memref<1x20x32xf32>
  return
}

...
#transform_map1 = #rock.transform_map<affine_map<(d0, d1) -> (0, d1, d0)> by [<Merge{1, 32} ["nr"] at [0] -> ["1", "z"] at [0, 2]>, <PassThrough ["r"] at [1] -> ["0"] at [1]>] bounds = [32, 20] -> [1, 20, 32]>
...
func.func @rock_threadwise_memcopy_pad(%input : memref<1x17x29xf32>,  %output : memref<1x17x29xf32>) attributes{rock.arch = "##TOKEN_ARCH##", block_size = 20 : i32, grid_size = 8 : i32, rock.kernel} {
  ...
  rock.threadwise_write_all {forceUnroll, useIndexDiffs} %input_reg -> [#transform_map5, #transform_map4](%ws_lds)[%tid] by set : memref<4xf32, #gpu.address_space<private>> -> memref<4x17xf32, #gpu.address_space<workgroup>>
  rock.threadwise_read_into {forceUnroll, useIndexDiffs}
  ...
  rock.threadwise_write_all {forceUnroll, useIndexDiffs} %output_reg -> [#transform_map3, #transform_map2, #transform_map1, #transform_map0](%output)[%bid, %tid] by set : memref<4xf32, #gpu.address_space<private>> -> memref<1x17x29xf32>
  return
}

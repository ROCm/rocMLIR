...
// VECTORIZE: affine.for %{{.*}} = 0 to 4 step 2
...
// LLVM: %[[load:.*]] = load <4 x float>, ptr addrspace(3) {{.*}}, align 16
// LLVM-NEXT: %[[extract0:.*]] = extractelement <4 x float> %[[load]], i64 0
// LLVM-NEXT: %[[extract1:.*]] = extractelement <4 x float> %[[load]], i64 1
// LLVM-NEXT: tail call <2 x half> @llvm.amdgcn.cvt.pkrtz(float %[[extract0]], float %[[extract1]])
// LLVM-NEXT: %[[extract2:.*]] = extractelement <4 x float> %[[load]], i64 2
// LLVM-NEXT: %[[extract3:.*]] = extractelement <4 x float> %[[load]], i64 3
// LLVM-NEXT: tail call <2 x half> @llvm.amdgcn.cvt.pkrtz(float %[[extract2]], float %[[extract3]])
// ASM: v_pk_add_f16 {{.*}}, {{.*}}, {{.*}}
module {

...
//===----------------------------------------------------------------------===//
// Test: 1D, no transforms, no iterArgs, default flags, explicit bounds/strides
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// Test: 2D, with transforms, explicit strides omitted (should default to 1)
//===----------------------------------------------------------------------===//
// CHECK-LABEL: func @two_dim_transform_default_strides
// CHECK: rock.transforming_for
func.func @two_dim_transform_default_strides(%i0: index, %i1: index) {
  %for = "rock.transforming_for"(%i0, %i1) ({
    "rock.yield"() : () -> ()
  }) {
    transforms = [
      [#rock.transform_map<PassThrough ["x"] at [0] -> ["y"] at [0], affine_map<(d0) -> (d0)>, dense<[5]> : tensor<1xi64>, dense<[5]> : tensor<1xi64>>],
      []
    ],
    bounds = [5, 7]
    // no strides attribute
  } : (index, index) -> ()
  return
}

//===----------------------------------------------------------------------===//
// Test: 1D, no transforms, no iterArgs, explicit strides omitted (should default to 1)
//===----------------------------------------------------------------------===//
// CHECK-LABEL: func @one_dim_no_transform_default_stride
// CHECK: rock.transforming_for
func.func @one_dim_no_transform_default_stride(%init: index) {
  %for = "rock.transforming_for"(%init) ({
    "rock.yield"() : () -> ()
  }) {
    transforms = [ [] ],

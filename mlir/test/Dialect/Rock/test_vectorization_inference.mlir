...
#transform_unmerge_3 = #rock.transform_map<affine_map<(d0, d1, d2, d3) -> (d3 + 2*d2 + 4*d1 + 128*d0)>
  by [<Embed{1, 2, 4, 128} ["Wo", "Ho", "K", "N"] at [3, 2, 1, 0] -> ["x"] at [0]>]
  bounds = [2, 2, 32, 128] -> [16384]>
...
func.func @test_vectorization() {
  ...
  // // // TODO: make it work with embed as well
  // // // CHECK-NEXT: result = 1 
  // // %31 = "get_length"() {transforms = [#transform_merge_2, #transform_shuffle, #transform_unmerge_3], in_dim = 1 : index, max_len = 4: index} : () -> (memref<16384xf32>)
  func.return
}

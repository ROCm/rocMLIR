...
//===----------------------------------------------------------------------===//
// Test: getAcceptingViewOperands returns only the first operand (source)
//===----------------------------------------------------------------------===//
...
func.func @accepting_view_operand_basic(%src: memref<16xf32, 5>, %dst: memref<16xf32, 0>) {
  // The first operand (%src) is the only accepting view operand.
...
func.func @accepting_view_operand_extra(%src: memref<8x8xf16, 5>, %dst: memref<8x8xf16, 0>, %idx: index) {
  // The first operand (%src) is still the only accepting view operand.

...
module {
  func.func @test(%arg0: memref<3xf16> {mhal.read_access}, %arg1: memref<1x128x56x56xf16> {mhal.read_access}, %arg2: memref<128x128x3x3xi8> {mhal.read_access}, %arg3: memref<1x128x28x28xf16> {mhal.write_access}) attributes {block_size = 64 : i32, grid_size = 392 : i32, kernel, original_func = @test} {
    ...
    scf.for %arg4 = %c0 to %c36 step %c1 {
      ...
      rock.stage {
        rock.threadwise_write_all features =  mfma|dot|atomic_add|atomic_add_f16 {forceUnroll, useIndexDiffs} %alloc_2 -> [](%alloc_7) [%13] by  set : memref<8xf16, #gpu.address_space<private>> -> memref<64x8xvector<8xf16>, #gpu.address_space<workgroup>>
        rock.yield
      } {name = "LDSWrite"}
      rock.stage {
        %21 = rock.workitem_id : index
        rock.threadwise_accel_gemm %20 += %18 * %19 at[%21, %21, %21] features =  mfma|dot|atomic_add|atomic_add_f16 {arch = "gfx90a", params = #rock.xdlops_gemm_derived_params<kpackPerBlock = 4, mPerBlock = 16, nPerBlock = 16, kpack = 8, mPerWave = 16, nPerWave = 16, mnPerXdl = 16, splitKFactor = 1, scheduleVersion = 1, outputSwizzle = 2, forceUnroll = true>} : memref<1x1xvector<4xf32>, #gpu.address_space<private>> += memref<1x2xvector<4xf16>, #gpu.address_space<private>> * memref<1x2xvector<4xf16>, #gpu.address_space<private>>
        rock.yield
      } {name = "MMA"}

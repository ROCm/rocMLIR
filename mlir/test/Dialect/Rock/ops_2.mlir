...
func.func @rock_buffer_load_f32(%source : memref<?x?x?xf32>, %sc0 : index, %sc1 : index, %sc2 : index) -> f32  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xf32>, index, index, index -> f32

func.func @rock_buffer_load_2xf32(%source : memref<?x?x?xf32>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<2xf32>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xf32>, index, index, index -> vector<2xf32>

func.func @rock_buffer_load_4xf32(%source : memref<?x?x?xf32>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<4xf32>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xf32>, index, index, index -> vector<4xf32>

// f16 tests.

func.func @rock_buffer_load_f16(%source : memref<?x?x?xf16>, %sc0 : index, %sc1 : index, %sc2 : index) -> f16  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xf16>, index, index, index -> f16

func.func @rock_buffer_load_2xf16(%source : memref<?x?x?xf16>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<2xf16>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xf16>, index, index, index -> vector<2xf16>

func.func @rock_buffer_load_4xf16(%source : memref<?x?x?xf16>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<4xf16>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xf16>, index, index, index -> vector<4xf16>

func.func @rock_buffer_load_8xf16(%source : memref<?x?x?xf16>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<8xf16>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xf16>, index, index, index -> vector<8xf16>

// bf16 tests.

func.func @rock_buffer_load_bf16(%source : memref<?x?x?xbf16>, %sc0 : index, %sc1 : index, %sc2 : index) -> bf16  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xbf16>, index, index, index -> bf16

func.func @rock_buffer_load_2xbf16(%source : memref<?x?x?xbf16>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<2xbf16>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xbf16>, index, index, index -> vector<2xbf16>

func.func @rock_buffer_load_4xbf16(%source : memref<?x?x?xbf16>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<4xbf16>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xbf16>, index, index, index -> vector<4xbf16>

func.func @rock_buffer_load_8xbf16(%source : memref<?x?x?xbf16>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<8xbf16>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2]  {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xbf16>, index, index, index -> vector<8xbf16>

// i8 tests.

func.func @rock_buffer_load_i8(%source : memref<?x?x?xi8>, %sc0 : index, %sc1 : index, %sc2 : index) -> i8  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xi8>, index, index, index -> i8

func.func @rock_buffer_load_4xi8(%source : memref<?x?x?xi8>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<4xi8>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xi8>, index, index, index -> vector<4xi8>

// i32 tests.

func.func @rock_buffer_load_i32(%source : memref<?x?x?xi32>, %sc0 : index, %sc1 : index, %sc2 : index) -> i32  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xi32>, index, index, index -> i32

func.func @rock_buffer_load_2xi32(%source : memref<?x?x?xi32>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<2xi32>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xi32>, index, index, index -> vector<2xi32>

func.func @rock_buffer_load_4xi32(%source : memref<?x?x?xi32>, %sc0 : index, %sc1 : index, %sc2 : index) -> vector<4xi32>  {
  %result = rock.buffer_load %source[%sc0, %sc1, %sc2] {
    leftOobDims = [], rightOobDims = []
...
// CHECK: %{{.*}} = rock.buffer_load %{{.*}}[%{{.*}}, %{{.*}}, %{{.*}}] {{.*}} : memref<?x?x?xi32>, index, index, index -> vector<4xi32>
...
func.func @rock_buffer_store_f32(%data: f32, %dest: memref<1x1x1x1x16xf32>, %idx: index) {
...
func.func @rock_buffer_store_2xf32(%data: vector<2xf32>, %dest: memref<1x1x1x1x16xf32>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_4xf32(%data: vector<4xf32>, %dest: memref<1x1x1x1x16xf32>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_f16(%data: f16, %dest: memref<1x1x1x1x16xf16>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_2xf16(%data: vector<2xf16>, %dest: memref<1x1x1x1x16xf16>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_4xf16(%data: vector<4xf16>, %dest: memref<1x1x1x1x16xf16>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_8xf16(%data: vector<8xf16>, %dest: memref<1x1x1x1x16xf16>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_bf16(%data: bf16, %dest: memref<1x1x1x1x16xbf16>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_2xbf16(%data: vector<2xbf16>, %dest: memref<1x1x1x1x16xbf16>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_4xbf16(%data: vector<4xbf16>, %dest: memref<1x1x1x1x16xbf16>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_8xbf16(%data: vector<8xbf16>, %dest: memref<1x1x1x1x16xbf16>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_i8(%data: i8, %dest: memref<1x1x1x1x16xi8>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_4xi8(%data: vector<4xi8>, %dest: memref<1x1x1x1x16xi8>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
// i32 tests.
func.func @rock_buffer_store_i32(%data: i32, %dest: memref<1x1x1x1x16xi32>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_2xi32(%data: vector<2xi32>, %dest: memref<1x1x1x1x16xi32>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_buffer_store_4xi32(%data: vector<4xi32>, %dest: memref<1x1x1x1x16xi32>, %idx: index) {
  %c0 = arith.constant 0 : index
  rock.buffer_store set %data -> %dest[%c0, %c0, %c0, %c0, %idx]
    {leftOobDims = [], rightOobDims = []}
...
func.func @rock_global_load(%source : memref<?x?x?x?x?xf32>) -> vector<8xf32> {

...
func.func @gather_negative_indices(%data: !migraphx.shaped<5x4xf16, 4x1>) -> !migraphx.shaped<3x4xf16, 4x1> {
  ...
  // expected-error @+1 {{'migraphx.gather' op indices must be non-negative (MIGraphX should normalize negative indices), found negative index value}}
...
func.func @scatter_none_negative_indices(
  ...
  // expected-error @+1 {{'migraphx.scatter_none' op indices must be non-negative (MIGraphX should normalize negative indices), found negative index value}}
  %0 = migraphx.scatter_none %data, %indices, %updates {axis = 0}

...
// RUN: miopen-gen  -fil_layout=gkcyx -in_layout=ngchw -out_layout=ngkhw -batchsize=32 -in_channels=32 -out_channels=32 -in_h=14 -in_w=14 -fil_h=1 -fil_w=1 --dilation_h=1 --dilation_w=1 --padding_h=1 --padding_w=1 --conv_stride_h=2 --conv_stride_w=2 --groupsize=1  --operation=conv2d_bwd_data | miopen-opt -miopen-affix-params -miopen-lowering | FileCheck %s --check-prefix=STRIDE2_1x1_LOWERING

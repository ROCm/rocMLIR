// RUN: miopen-gen -p --host %s | FileCheck %s --check-prefix=HARNESS
// RUN: miopen-gen -p --host %s | mlir-miopen-driver -c | FileCheck %s --check-prefix=LOWERING
// RUN: miopen-gen -p --host %s | mlir-miopen-driver -c | mlir-rocm-runner --shared-libs=%rocm_wrapper_library_dir/librocm-runtime-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E
...
// LOWERING: module
// LOWERING: gpu.launch_func  @miopen_conv2d_gkcyx_ngchw_ngkhw_0_module::@miopen_conv2d_gkcyx_ngchw_ngkhw_0  blocks in (%{{.*}}, %{{.*}}, %{{.*}}) threads in (%{{.*}}, %{{.*}}, %{{.*}}) args(%{{.*}} : memref<1x128x8x3x3xf32>, %{{.*}} : memref<128x1x8x32x32xf32>, %{{.*}} : memref<128x1x128x30x30xf32>)
...
func @main() {
  ...
  call @miopen_conv2d_gkcyx_ngchw_ngkhw_0(%filter, %input, %output) : (memref<1x128x8x3x3xf32>, memref<128x1x8x32x32xf32>, memref<128x1x128x30x30xf32>) -> ()

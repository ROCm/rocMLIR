...
// RUN: miopen-gen -p -fil_layout=gcyxk -in_layout=gchwn -out_layout=gkhwn --host %s | mlir-miopen-driver -c | mlir-rocm-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%rocm_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E

func private @miopen_conv2d_gcyxk_gchwn_gkhwn_0(%filter : memref<1x8x3x3x128xf32>, %input : memref<1x8x32x32x128xf32>, %output : memref<1x128x30x30x128xf32>) -> ()

// HARNESS: module
// HARNESS: func @miopen_conv2d_gcyxk_gchwn_gkhwn_0([[FILTER_MEMREF:%.*]]: memref<1x8x3x3x128xf32>, [[INPUT_MEMREF:%.*]]: memref<1x8x32x32x128xf32>, [[OUTPUT_MEMREF:%.*]]: memref<1x128x30x30x128xf32>)
// LOWERING: module

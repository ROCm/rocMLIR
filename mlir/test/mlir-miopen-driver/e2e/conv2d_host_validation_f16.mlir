...
// RUN: mlir-miopen-driver -t f16 -pv -fil_layout=yxck -in_layout=nhwc -out_layout=nhwk -in_channels=16 -batchsize=64 -out_channels=128 -in_h=16 -in_w=16 -fil_h=3 -fil_w=3 -p=false -c | mlir-rocm-runner --shared-lib%rocm_wrapper_library_dir/b/librocm-runtime-wrappers%shlibex%linalg_test_lib_dir/b/libmlir_runner_utils%shlibext --entry-point-result=vo | FileCheck %s --check-prefix=CHECK_ISSUE_41_5

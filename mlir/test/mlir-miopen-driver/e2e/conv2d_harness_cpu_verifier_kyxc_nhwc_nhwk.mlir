...
module {
  func @main() {
    ...
    call @mcpuMemset5DFloat(%5, %cst_0) : (memref<?x?x?x?x?xf32>, f32) -> ()
    ...
    call @mcpuMemset5DFloat(%7, %cst) : (memref<?x?x?x?x?xf32>, f32) -> ()

    %8 = alloc() : memref<128x32x32x1x8xf32>
    %9 = memref_cast %8 : memref<128x32x32x1x8xf32> to memref<?x?x?x?x?xf32>
    call @mcpuMemset5DFloat(%9, %cst) : (memref<?x?x?x?x?xf32>, f32) -> ()

    %10 = alloc() : memref<128x30x30x1x128xf32>
    %11 = memref_cast %10 : memref<128x30x30x1x128xf32> to memref<?x?x?x?x?xf32>
    call @mcpuMemset5DFloat(%11, %cst_0) : (memref<?x?x?x?x?xf32>, f32) -> ()

    // launch cpu convolution
    call @conv2d_host(%6, %8, %10) : (memref<1x128x3x3x8xf32>, memref<128x32x32x1x8xf32>, memref<128x30x30x1x128xf32>) -> ()

    // verity results
    call @verify_results(%10, %2) : (memref<128x30x30x1x128xf32>, memref<128x30x30x1x128xf32>) -> ()
    ...
    dealloc %8 : memref<128x32x32x1x8xf32>
    dealloc %10 : memref<128x30x30x1x128xf32>
    return
  }

  func private @mcpuMemset5DFloat(memref<?x?x?x?x?xf32>, f32)

  func @gpu_conv(%arg0: memref<1x128x3x3x8xf32>, %arg1: memref<128x32x32x1x8xf32>, %arg2: memref<128x30x30x1x128xf32>) {
    %0 = memref_cast %arg0 : memref<1x128x3x3x8xf32> to memref<?x?x?x?x?xf32>
    ...
    // launch kernel.
    %6 = memref_cast %3 : memref<?x?x?x?x?xf32> to memref<1x128x3x3x8xf32>
  ...
  func @conv2d_host(%arg0: memref<1x128x3x3x8xf32>, %arg1: memref<128x32x32x1x8xf32>, %arg2: memref<128x30x30x1x128xf32>) {
    ...
    store %k, %6[%c1] : memref<5xi8>
    store %y, %6[%c2] : memref<5xi8>
    store %x, %6[%c3] : memref<5xi8>
    store %c, %6[%c4] : memref<5xi8>

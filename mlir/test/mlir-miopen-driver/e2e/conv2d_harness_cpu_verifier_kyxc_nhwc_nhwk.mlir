// RUN: mlir-miopen-driver -p -fil_layout=kyxc -in_layout=nhwc -out_layout=nhwk --host %s -c | mlir-rocm-runner --shared-libs=%rocm_wrapper_library_dir/librocm-runtime-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E
...
module {
  ...
  func @gpu_conv(%arg0: memref<128x3x3x8xf32>, %arg1: memref<128x32x32x8xf32>, %arg2: memref<128x30x30x128xf32>) {
    ...
    // launch kernel.
    %6 = memref_cast %3 : memref<?x?x?x?xf32> to memref<128x3x3x8xf32>
    %7 = memref_cast %4 : memref<?x?x?x?xf32> to memref<128x32x32x8xf32>
    %8 = memref_cast %5 : memref<?x?x?x?xf32> to memref<128x30x30x128xf32>
    call @conv2d(%6, %7, %8) : (memref<128x3x3x8xf32>, memref<128x32x32x8xf32>, memref<128x30x30x128xf32>) -> ()
  ...
  func private @mgpuMemAlloc4DFloat(memref<?x?x?x?xf32>) -> memref<?x?x?x?xf32>
  func private @mgpuMemCopy4DFloat(memref<?x?x?x?xf32>, memref<?x?x?x?xf32>, i32)

  func @conv2d(%arg0: memref<128x3x3x8xf32>, %arg1: memref<128x32x32x8xf32>, %arg2: memref<128x30x30x128xf32>) {
    return
  }

  func private @mgpuMemDealloc4DFloat(memref<?x?x?x?xf32>)
  ...
  func @conv2d_host(%arg0: memref<128x3x3x8xf32>, %arg1: memref<128x32x32x8xf32>, %arg2: memref<128x30x30x128xf32>) {
    ...
    // store layouts
    store %k, %6[%c0] : memref<4xi8>
    store %y, %6[%c1] : memref<4xi8>
    store %x, %6[%c2] : memref<4xi8>
    store %c, %6[%c3] : memref<4xi8>

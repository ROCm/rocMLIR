...
// RUN: mlir-miopen-driver -p -fil_layout=yxck -in_layout=hwcn -out_layout=hwkn -c --host %s | mlir-rocm-runner --shared-libs=%rocm_wrapper_library_dir/librocm-runtime-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E

func @conv2d(%filter : memref<3x3x8x128xf32>, %input : memref<32x32x8x128xf32>, %output : memref<30x30x128x128xf32>) {
  // Convolution host-side logic would be populated here.
  return
}
// HARNESS: module
// HARNESS: func @conv2d([[FILTER_MEMREF:%.*]]: memref<3x3x8x128xf32>, [[INPUT_MEMREF:%.*]]: memref<32x32x8x128xf32>, [[OUTPUT_MEMREF:%.*]]: memref<30x30x128x128xf32>)
// LOWERING: module
// LOWERING: func @conv2d([[FILTER_MEMREF:%.*]]: memref<3x3x8x128xf32>, [[INPUT_MEMREF:%.*]]: memref<32x32x8x128xf32>, [[OUTPUT_MEMREF:%.*]]: memref<30x30x128x128xf32>)
// LOWERING: "gpu.launch_func"(%{{.*}}, %{{.*}}, %{{.*}}, %{{.*}}, %{{.*}}, %{{.*}}, [[FILTER_MEMREF]], [[INPUT_MEMREF]], [[OUTPUT_MEMREF]]) {kernel = @miopen_kernel_module::@miopen_conv2d_kcyx_nchw_nkhw} : (index, index, index, index, index, index, memref<3x3x8x128xf32>, memref<32x32x8x128xf32>, memref<30x30x128x128xf32>) -> ()

func @main() {
  // allocate CPU memory.
  %0 = alloc() : memref<3x3x8x128xf32>
  %1 = alloc() : memref<32x32x8x128xf32>
  %2 = alloc() : memref<30x30x128x128xf32>

  %3 = memref_cast %0 : memref<3x3x8x128xf32> to memref<?x?x?x?xf32>
  %4 = memref_cast %1 : memref<32x32x8x128xf32> to memref<?x?x?x?xf32>
  ...
  %input = memref_cast %7 : memref<?x?x?x?xf32> to memref<32x32x8x128xf32>
  %output = memref_cast %8 : memref<?x?x?x?xf32> to memref<30x30x128x128xf32>
  call @conv2d(%filter, %input, %output) : (memref<3x3x8x128xf32>, memref<32x32x8x128xf32>, memref<30x30x128x128xf32>) -> ()
  ...
  dealloc %1 : memref<32x32x8x128xf32>

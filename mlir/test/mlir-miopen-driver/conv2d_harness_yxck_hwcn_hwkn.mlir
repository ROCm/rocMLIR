...
// LOWERING: gpu.launch_func  @miopen_conv2d_gyxck_hwgcn_hwgkn_module::@miopen_conv2d_gyxck_hwgcn_hwgkn blocks in (%{{.*}}, %{{.*}}, %{{.*}}) threads in (%{{.*}}, %{{.*}}, %{{.*}}) args([[FILTER_MEMREF]] : memref<1x3x3x8x128xf32>, [[INPUT_MEMREF]] : memref<32x32x1x8x128xf32>, [[OUTPUT_MEMREF]] : memref<30x30x1x128x128xf32>)

func @main() {
  // allocate CPU memory.
  %0 = alloc() : memref<1x3x3x8x128xf32>
  %1 = alloc() : memref<32x32x1x8x128xf32>
  %2 = alloc() : memref<30x30x1x128x128xf32>

  %3 = memref_cast %0 : memref<1x3x3x8x128xf32> to memref<?x?x?x?x?xf32>
  %4 = memref_cast %1 : memref<32x32x1x8x128xf32> to memref<?x?x?x?x?xf32>
  %5 = memref_cast %2 : memref<30x30x1x128x128xf32> to memref<?x?x?x?x?xf32>
  ...
  // launch kernel.
  %filter = memref_cast %6 : memref<?x?x?x?x?xf32> to memref<1x3x3x8x128xf32>
  %input = memref_cast %7 : memref<?x?x?x?x?xf32> to memref<32x32x1x8x128xf32>
  %output = memref_cast %8 : memref<?x?x?x?x?xf32> to memref<30x30x1x128x128xf32>
  ...
  dealloc %1 : memref<32x32x1x8x128xf32>
  dealloc %2 : memref<30x30x1x128x128xf32>
...
// LOWERING: gpu.module @miopen_conv2d_gyxck_hwgcn_hwgkn_module
// LOWERING: gpu.func @miopen_conv2d_gyxck_hwgcn_hwgkn
// TBD. Add more verifying logic.

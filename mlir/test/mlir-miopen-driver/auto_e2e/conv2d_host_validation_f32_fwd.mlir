...
// RUN: mlir-miopen-driver %pv %random_data %xdlops -fil_layout=kcyx -in_layout=nchw -out_layout=nkhw -batchsize=16 -in_channels=1 -out_channels=16 -in_h=14 -in_w=14 -fil_h=14 -fil_w=14 --dilation_h=1 --dilation_w=1 --padding_h=1 --padding_w=1 --conv_stride_h=1 --conv_stride_w=1 --operation=conv2d -p=false -c | mlir-rocm-runner --shared-libs=%rocm_wrapper_library_dir/librocm-runtime-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=CHECK_ISSUE_136_4

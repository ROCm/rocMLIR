...
// RUN: mlir-miopen-driver -p -fil_layout=gcyxk -in_layout=gchwn -out_layout=gkhwn -c --host %s | mlir-rocm-runner --shared-libs=%rocm_wrapper_library_dir/librocm-runtime-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E
...
// LOWERING: gpu.launch_func  @miopen_conv2d_gcyxk_gchwn_gkhwn_module::@miopen_conv2d_gcyxk_gchwn_gkhwn blocks in (%{{.*}}, %{{.*}}, %{{.*}}) threads in (%{{.*}}, %{{.*}}, %{{.*}}) args([[FILTER_MEMREF]] : memref<1x8x3x3x128xf32>, [[INPUT_MEMREF]] : memref<1x8x32x32x128xf32>, [[OUTPUT_MEMREF]] : memref<1x128x30x30x128xf32>)

func @main() {
  // allocate CPU memory.
  %0 = alloc() : memref<1x8x3x3x128xf32>
  %1 = alloc() : memref<1x8x32x32x128xf32>
  %2 = alloc() : memref<1x128x30x30x128xf32>

  %3 = memref_cast %0 : memref<1x8x3x3x128xf32> to memref<?x?x?x?x?xf32>
  %4 = memref_cast %1 : memref<1x8x32x32x128xf32> to memref<?x?x?x?x?xf32>
  %5 = memref_cast %2 : memref<1x128x30x30x128xf32> to memref<?x?x?x?x?xf32>
  ...
  // launch kernel.
  %filter = memref_cast %6 : memref<?x?x?x?x?xf32> to memref<1x8x3x3x128xf32>
  %input = memref_cast %7 : memref<?x?x?x?x?xf32> to memref<1x8x32x32x128xf32>
  %output = memref_cast %8 : memref<?x?x?x?x?xf32> to memref<1x128x30x30x128xf32>
  ...
  dealloc %0 : memref<1x8x3x3x128xf32>
  dealloc %1 : memref<1x8x32x32x128xf32>
  dealloc %2 : memref<1x128x30x30x128xf32>
...
// LOWERING: gpu.module @miopen_conv2d_gcyxk_gchwn_gkhwn_module
// LOWERING: gpu.func @miopen_conv2d_gcyxk_gchwn_gkhwn
// TBD. Add more verifying logic.

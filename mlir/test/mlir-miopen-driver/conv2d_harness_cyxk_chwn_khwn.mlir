...
// RUN: mlir-miopen-driver -p -fil_layout=cyxk -in_layout=chwn -out_layout=khwn -c --host %s | mlir-rocm-runner --shared-libs=%rocm_wrapper_library_dir/librocm-runtime-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E

func @conv2d(%filter : memref<8x3x3x128xf32>, %input : memref<8x32x32x128xf32>, %output : memref<128x30x30x128xf32>) {
...
func @main() {
  // allocate CPU memory.
  %0 = alloc() : memref<8x3x3x128xf32>
  %1 = alloc() : memref<8x32x32x128xf32>
  %2 = alloc() : memref<128x30x30x128xf32>

  %3 = memref_cast %0 : memref<8x3x3x128xf32> to memref<?x?x?x?xf32>
  %4 = memref_cast %1 : memref<8x32x32x128xf32> to memref<?x?x?x?xf32>
  ...
  // launch kernel.
  %filter = memref_cast %6 : memref<?x?x?x?xf32> to memref<8x3x3x128xf32>
  %input = memref_cast %7 : memref<?x?x?x?xf32> to memref<8x32x32x128xf32>
  %output = memref_cast %8 : memref<?x?x?x?xf32> to memref<128x30x30x128xf32>
  call @conv2d(%filter, %input, %output) : (memref<8x3x3x128xf32>, memref<8x32x32x128xf32>, memref<128x30x30x128xf32>) -> ()
  ...
  dealloc %0 : memref<8x3x3x128xf32>
  dealloc %1 : memref<8x32x32x128xf32>

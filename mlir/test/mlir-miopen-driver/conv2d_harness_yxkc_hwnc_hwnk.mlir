// RUN: mlir-miopen-driver -p -fil_layout=gyxkc -in_layout=hwngc -out_layout=hwngk --host %s | FileCheck %s --check-prefix=HARNESS
// RUN: mlir-miopen-driver -pc -fil_layout=gyxkc -in_layout=hwngc -out_layout=hwngk --host %s | FileCheck %s --check-prefix=LOWERING
// RUN: mlir-miopen-driver -p -fil_layout=gyxkc -in_layout=hwngc -out_layout=hwngk -c --host %s | mlir-rocm-runner --shared-libs=%rocm_wrapper_library_dir/librocm-runtime-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E

func @conv2d(%filter : memref<1x3x3x128x8xf32>, %input : memref<32x32x128x1x8xf32>, %output : memref<30x30x128x1x128xf32>) {
...
// LOWERING: gpu.launch_func  @miopen_conv2d_gyxkc_hwngc_hwngk_0_module::@miopen_conv2d_gyxkc_hwngc_hwngk_0 blocks in (%{{.*}}, %{{.*}}, %{{.*}}) threads in (%{{.*}}, %{{.*}}, %{{.*}}) args([[FILTER_MEMREF]] : memref<1x3x3x128x8xf32>, [[INPUT_MEMREF]] : memref<32x32x128x1x8xf32>, [[OUTPUT_MEMREF]] : memref<30x30x128x1x128xf32>)

func @main() {
  // allocate CPU memory.
  %0 = alloc() : memref<1x3x3x128x8xf32>
  %1 = alloc() : memref<32x32x128x1x8xf32>
  %2 = alloc() : memref<30x30x128x1x128xf32>

  %3 = memref_cast %0 : memref<1x3x3x128x8xf32> to memref<?x?x?x?x?xf32>
  %4 = memref_cast %1 : memref<32x32x128x1x8xf32> to memref<?x?x?x?x?xf32>
  %5 = memref_cast %2 : memref<30x30x128x1x128xf32> to memref<?x?x?x?x?xf32>
  ...
  // launch kernel.
  %filter = memref_cast %6 : memref<?x?x?x?x?xf32> to memref<1x3x3x128x8xf32>
  %input = memref_cast %7 : memref<?x?x?x?x?xf32> to memref<32x32x128x1x8xf32>
  %output = memref_cast %8 : memref<?x?x?x?x?xf32> to memref<30x30x128x1x128xf32>
  call @conv2d(%filter, %input, %output) : (memref<1x3x3x128x8xf32>, memref<32x32x128x1x8xf32>, memref<30x30x128x1x128xf32>) -> ()
  ...
  dealloc %0 : memref<1x3x3x128x8xf32>
  dealloc %1 : memref<32x32x128x1x8xf32>
  dealloc %2 : memref<30x30x128x1x128xf32>

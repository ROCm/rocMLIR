...
// CHECK-NEXT:    %cst = arith.constant dense<0.000000e+00> : tensor<32xf32>
// CHECK-NEXT:    %0 = "tosa.depthwise_conv2d"(%arg0, %arg2, %cst) {dilation = [1, 1], pad = [1, 1, 1, 1], stride = [1, 1]} : (tensor<1x112x112x32xf32>, tensor<3x3x32x1xf32>, tensor<32xf32>) -> tensor<1x112x112x32xf32>
// CHECK-NEXT:    %token, %results = async.launch @mobilenetv1_outlined_part_0 (%0, %arg3) : (tensor<1x112x112x32xf32>, tensor<64x1x1x32xf32>) -> tensor<1x112x112x64xf32>
// CHECK-NEXT:    async.await %token : !async.token
// CHECK:    return %results : tensor<1x112x112x64xf32>

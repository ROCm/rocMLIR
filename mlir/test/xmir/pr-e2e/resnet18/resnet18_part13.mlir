// RUN: rocmlir-gen -fut forward__part_13 --arch %arch --clone-harness %s | rocmlir-driver -host-pipeline highlevel | rocmlir-gen -ph -fut forward__part_13_wrapper --verifier clone - | rocmlir-driver -host-pipeline mhal,runner -kernel-pipeline full -targets %arch | xmir-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext,%linalg_test_lib_dir/libmlir_c_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_async_runtime%shlibext --entry-point-result=void | FileCheck %s

// ALLOW_RETRIES: 2
// CHECK: [1 1 1]

module {
  func.func private @forward__part_13(%arg0: tensor<1x64x56x56xf32> {mhal.read_access}, %arg1: tensor<128x1x1xf32> {mhal.read_access}, %arg2: tensor<1x128x28x28xf32> {mhal.read_access}) -> (tensor<1x128x28x28xf32> {mhal.write_access}) {
    %1 = tosa.transpose %arg0 {perms = array<i32: 0, 2, 3, 1>} : (tensor<1x64x56x56xf32>) -> tensor<1x56x56x64xf32>
    %2 = "tosa.const"() <{value = dense<0.015915595> : tensor<128x1x1x64xf32>}> : () -> tensor<128x1x1x64xf32>
    %3 = "tosa.const"() <{value = dense<0.000000e+00> : tensor<128xf32>}> : () -> tensor<128xf32>
    %5 = "tosa.const"() <{value = dense<-0.211282596> : tensor<1x128x1x1xf32>}> : () -> tensor<1x128x1x1xf32>
    ...
    %7 = "tosa.const"() <{value = dense<0.333436757> : tensor<1x128x1x1xf32>}> : () -> tensor<1x128x1x1xf32>
    ...
    %9 = tosa.conv2d %1, %2, %3, %input_zp, %weight_zp {acc_type = f32, dilation = array<i64: 1, 1>, pad = array<i64: 0, 0, 0, 0>, stride = array<i64: 2, 2>} : (tensor<1x56x56x64xf32>, tensor<128x1x1x64xf32>, tensor<128xf32>, tensor<1xf32>, tensor<1xf32>) -> tensor<1x28x28x128xf32>

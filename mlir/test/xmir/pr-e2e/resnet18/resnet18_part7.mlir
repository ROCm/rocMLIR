// RUN: rocmlir-gen -fut forward__part_7 --arch %arch --clone-harness %s | rocmlir-driver -host-pipeline highlevel | rocmlir-gen -ph -fut forward__part_7_wrapper --verifier clone - | rocmlir-driver -host-pipeline mhal,runner -kernel-pipeline full -targets %arch | xmir-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext,%linalg_test_lib_dir/libmlir_c_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_async_runtime%shlibext --entry-point-result=void | FileCheck %s

// ALLOW_RETRIES: 2
// CHECK: [1 1 1]

module {
  func.func private @forward__part_7(%arg0: tensor<1x256x14x14xf32> {mhal.read_access}, %arg1: tensor<256x1x1xf32> {mhal.read_access}) -> (tensor<1x256x14x14xf32> {mhal.write_access}) {
    %1 = tosa.transpose %arg0 {perms = array<i32: 0, 2, 3, 1>} : (tensor<1x256x14x14xf32>) -> tensor<1x14x14x256xf32>
    %2 = "tosa.const"() <{value = dense<0.0483669564> : tensor<256x3x3x256xf32>}> : () -> tensor<256x3x3x256xf32>
    %3 = "tosa.const"() <{value = dense<0.000000e+00> : tensor<256xf32>}> : () -> tensor<256xf32>
    %5 = "tosa.const"() <{value = dense<-0.102452755> : tensor<1x256x1x1xf32>}> : () -> tensor<1x256x1x1xf32>
    ...
    %7 = "tosa.const"() <{value = dense<0.248004287> : tensor<1x256x1x1xf32>}> : () -> tensor<1x256x1x1xf32>
    %8 = "tosa.const"() <{value = dense<-0.133214399> : tensor<1x256x1x1xf32>}> : () -> tensor<1x256x1x1xf32>

...
module {
  func.func private @forward__part_5(%arg0: tensor<1x256x14x14xf32> {mhal.read_access}, %arg1: tensor<512x1x1xf32> {mhal.read_access}) -> (tensor<1x512x7x7xf32> {mhal.write_access}) {
    ...
    %2 = "tosa.const"() <{value = dense<-0.0116454642> : tensor<512x3x3x256xf32>}> : () -> tensor<512x3x3x256xf32>
    %3 = "tosa.const"() <{value = dense<0.000000e+00> : tensor<512xf32>}> : () -> tensor<512xf32>
    %5 = "tosa.const"() <{value = dense<-0.112398677> : tensor<1x512x1x1xf32>}> : () -> tensor<1x512x1x1xf32>
    ...
    %7 = "tosa.const"() <{value = dense<0.242737785> : tensor<1x512x1x1xf32>}> : () -> tensor<1x512x1x1xf32>
    %8 = "tosa.const"() <{value = dense<-0.198634937> : tensor<1x512x1x1xf32>}> : () -> tensor<1x512x1x1xf32>
    ...
    %9 = tosa.conv2d %1, %2, %3, %input_zp, %weight_zp {acc_type = f32, dilation = array<i64: 1, 1>, pad = array<i64: 1, 1, 1, 1>, stride = array<i64: 2, 2>} : (tensor<1x14x14x256xf32>, tensor<512x3x3x256xf32>, tensor<512xf32>, tensor<1xf32>, tensor<1xf32>) -> tensor<1x7x7x512xf32>

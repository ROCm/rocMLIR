...
module {
  func.func private @forward__part_6(%arg0: tensor<1x256x14x14xf32> {mhal.read_access}, %arg1: tensor<256x1x1xf32> {mhal.read_access}, %arg2: tensor<1x256x14x14xf32> {mhal.read_access}) -> (tensor<1x256x14x14xf32> {mhal.write_access}) {
    ...
    %2 = "tosa.const"() <{value = dense<-0.0425682515> : tensor<256x3x3x256xf32>}> : () -> tensor<256x3x3x256xf32>
    %3 = "tosa.const"() <{value = dense<0.000000e+00> : tensor<256xf32>}> : () -> tensor<256xf32>
    %5 = "tosa.const"() <{value = dense<-0.0823119357> : tensor<1x256x1x1xf32>}> : () -> tensor<1x256x1x1xf32>
    ...
    %7 = "tosa.const"() <{value = dense<0.197083265> : tensor<1x256x1x1xf32>}> : () -> tensor<1x256x1x1xf32>
    %8 = "tosa.const"() <{value = dense<-0.0162449498> : tensor<1x256x1x1xf32>}> : () -> tensor<1x256x1x1xf32>

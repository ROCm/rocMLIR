...
module {
  func.func @mlir_dot_add(%arg0: memref<1x2x320xbf16>, %arg1: memref<1x2x1280xbf16>, %arg2: memref<1x1280x320xbf16>, %arg3: memref<1x2x1xbf16>) attributes {enable_splitk_for_tuning, kernel, mhal.arch = "amdgcn-amd-amdhsa:gfx1201"} {
    %alloc = memref.alloc() {alignment = 64 : i64} : memref<1x2x320xbf16>
    rock.gemm %alloc = %arg1 * %arg2 features =  mfma|dot|atomic_add|atomic_add_bf16 storeMethod =  set {arch = "amdgcn-amd-amdhsa:gfx1201"} : memref<1x2x320xbf16> = memref<1x2x1280xbf16> * memref<1x1280x320xbf16>
    ...
    rock.reduce  sum %0 into %alloc_1 features =  mfma|dot|atomic_add|atomic_add_bf16 {axis = 1 : index, blockSize = 256 : i32, gridSize = 2 : i32} : memref<2x320xbf16> into memref<2x1xbf16>

...
// CHECK: gfx942  120     -t f32 -out_datatype f32 -transA false -transB false -g 1 -m 128 -n 256 -k 64 -fusion_reduce count=2 sum:rank3:axis1:stride256 sum:rank3:axis2:stride1
...
#transform_map7 = #rock.transform_map<#map4 by [<Merge{1, 1, 256} ["dim0"] at [0] -> ["col0", "col1", "col2"] at [0, 1, 2]>] bounds = [256] -> [1, 1, 256]>
module {
  func.func private @gemm_multi_reduce_different_axes(%arg0: memref<8192xf32> {mhal.read_access}, %arg1: memref<16384xf32> {mhal.read_access}, %arg2: memref<128xf32> {mhal.read_access, mhal.write_access, rock.prefill = 0.000000e+00 : f32}, %arg3: memref<256xf32> {mhal.read_access, mhal.write_access, rock.prefill = 0.000000e+00 : f32}) attributes {arch = "gfx942", kernel, num_cu = 120 : i64} {
    ...
    // First reduction: sum on axis 1 (row reduction: 1x128x256 -> 1x1x256)
    %alloc_0 = memref.alloc() {alignment = 64 : i64} : memref<1x1x256xf32>
    rock.reduce  sum %alloc into %alloc_0 {axis = 1 : index, blockSize = 256 : i32, gridSize = 128 : i32} : memref<1x128x256xf32> into memref<1x1x256xf32>
    %6 = rock.transform %alloc_0 by #transform_map7 : memref<1x1x256xf32> to memref<256xf32>
    
    // Second reduction: sum on axis 2 (column reduction: 1x128x256 -> 1x128x1)
    %alloc_1 = memref.alloc() {alignment = 64 : i64} : memref<1x128x1xf32>
    rock.reduce  sum %alloc into %alloc_1 {axis = 2 : index, blockSize = 256 : i32, gridSize = 128 : i32} : memref<1x128x256xf32> into memref<1x128x1xf32>
    %7 = rock.transform %alloc_1 by #transform_map6 : memref<1x128x1xf32> to memref<128xf32>
    
    memref.copy %7, %arg2 : memref<128xf32> to memref<128xf32>
    memref.copy %6, %arg3 : memref<256xf32> to memref<256xf32>
    return
  }
}

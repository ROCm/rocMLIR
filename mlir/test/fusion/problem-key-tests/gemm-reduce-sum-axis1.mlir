...
// CHECK: gfx942  120     -t f32 -out_datatype f32 -transA false -transB false -g 1 -m 128 -n 256 -k 64 -fusion_reduce count=1 sum:rank3:axis1:stride256

#map = affine_map<(d0, d1, d2) -> (d1 * 256 + d2)>
#map1 = affine_map<(d0, d1, d2) -> (d1 * 64 + d2)>
#map2 = affine_map<(d0, d1, d2) -> (d1, d0, d2)>
#map3 = affine_map<(d0) -> (0, 0, d0)>
...
#transform_map6 = #rock.transform_map<#map3 by [<Merge{1, 1, 256} ["dim0"] at [0] -> ["col0", "col1", "col2"] at [0, 1, 2]>] bounds = [256] -> [1, 1, 256]>
module {
  func.func private @gemm_reduce_sum_axis1(%arg0: memref<8192xf32> {mhal.read_access}, %arg1: memref<16384xf32> {mhal.read_access}, %arg2: memref<256xf32> {mhal.read_access, mhal.write_access, rock.prefill = 0.000000e+00 : f32}) attributes {arch = "gfx942", kernel, num_cu = 120 : i64} {
    ...
    %6 = rock.transform %alloc_0 by #transform_map6 : memref<1x1x256xf32> to memref<256xf32>
    memref.copy %6, %arg2 : memref<256xf32> to memref<256xf32>
    return
  }
}

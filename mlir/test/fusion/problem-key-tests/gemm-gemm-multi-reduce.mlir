...
module {
  func.func private @gemm_gemm_multi_reduce(%a: !migraphx.shaped<1x128x64xf32, 8192x64x1> {mhal.read_access}, 
    ...
    %reduce1 = migraphx.reduce_sum %gemm1 {axes = [2 : i64]} : <1x128x128xf32, 16384x128x1> -> <1x128x1xf32, 128x1x1>
    
    // Second reduction: reduce_sum(x * x)
    %square = migraphx.mul %gemm1, %gemm1 : <1x128x128xf32, 16384x128x1>, <1x128x128xf32, 16384x128x1> -> <1x128x128xf32, 16384x128x1>
    %reduce2 = migraphx.reduce_sum %square {axes = [2 : i64]} : <1x128x128xf32, 16384x128x1> -> <1x128x1xf32, 128x1x1>

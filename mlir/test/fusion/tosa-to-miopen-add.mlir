...
// CHECK: linalg.generic {indexing_maps = [#map{{.*}}, #map{{.*}}], iterator_types = ["parallel", "parallel", "parallel"]} ins(%{{.*}}, %{{.*}} : memref<1x8x8xf32, 5>, memref<1x8x8xf32, 5>) outs(%{{.*}} : memref<1x8x8xf32, 5>)

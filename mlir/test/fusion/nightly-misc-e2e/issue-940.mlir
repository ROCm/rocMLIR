...
// CLONE: [1 1 1]
module {
  func.func @mlir_dot(%arg0: tensor<1x384x2304xf32>, %arg1: tensor<1x384x2304xf32>) -> tensor<1x12x384x384xf32> {
    %0 = "tosa.const"() {value = dense<1.250000e-01> : tensor<1xf32>} : () -> tensor<1xf32>
    %1 = migraphx.multibroadcast(%0) {out_dyn_dims = [], out_lens = [1, 12, 384, 384]} : (tensor<1xf32>) -> tensor<1x12x384x384xf32>
    %2 = migraphx.reshape(%arg0) {dims = [1, 384, 36, 64]} : (tensor<1x384x2304xf32>) -> tensor<1x384x36x64xf32>
    ...
    %5 = migraphx.reshape(%arg1) {dims = [1, 384, 36, 64]} : (tensor<1x384x2304xf32>) -> tensor<1x384x36x64xf32>
    ...
    %9 = migraphx.transpose(%4) {permutation = [0, 1, 3, 2]} : (tensor<1x12x384x64xf32>) -> tensor<1x12x64x384xf32>
    %10 = migraphx.transpose(%8) {permutation = [0, 1, 3, 2]} : (tensor<1x12x64x384xf32>) -> tensor<1x12x384x64xf32>
    %11 = migraphx.dot(%10, %9) : (tensor<1x12x384x64xf32>, tensor<1x12x64x384xf32>) -> tensor<1x12x384x384xf32>
    %12 = migraphx.transpose(%11) {permutation = [0, 1, 3, 2]} : (tensor<1x12x384x384xf32>) -> tensor<1x12x384x384xf32>
    %13 = migraphx.mul(%12, %1) : (tensor<1x12x384x384xf32>, tensor<1x12x384x384xf32>) -> tensor<1x12x384x384xf32>
    return %13 : tensor<1x12x384x384xf32>
  }
}
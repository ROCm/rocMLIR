...
module {
  func.func @mlir_dot_add(%arg0: memref<1x2x320xf16>, %arg1: memref<1x2x1280xf16>, %arg2: memref<1x1280x320xf16>, %arg3: memref<1x2x320xf16>) attributes {rock.enable_splitk_for_tuning, rock.kernel, mhal.arch = "amdgcn-amd-amdhsa:gfx1100:sramecc+:xnack-"} {
    %alloc = memref.alloc() alignment = 64 : memref<1x2x320xf16>
    rock.gemm %alloc = %arg1 * %arg2 features =  "mfma|dot|atomic_add" storeMethod =  set {rock.arch = "amdgcn-amd-amdhsa:gfx1100:sramecc+:xnack-"} : memref<1x2x320xf16> = memref<1x2x1280xf16> * memref<1x1280x320xf16>
    ...
    %alloc_0 = memref.alloc() alignment = 64 : memref<2x320xf16>
    linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%0, %1 : memref<2x320xf16>, memref<2x320xf16>) outs(%alloc_0 : memref<2x320xf16>) {

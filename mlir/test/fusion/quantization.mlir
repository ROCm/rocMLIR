...
func.func @test_conv_with_cast(
    ...
    %scale: tensor<8xf32>,
    %bias: tensor<8xi32>) -> tensor<1x8x8x8xf32> attributes {rock.kernel, rock.arch = "gfx906"}
{
    %zero = arith.constant dense<0> : tensor<8xi8>
    %input_zp = "tosa.const"() {values = dense<0> : tensor<1xi8>} : () -> tensor<1xi8>
    %weight_zp = "tosa.const"() {values = dense<0> : tensor<1xi8>} : () -> tensor<1xi8>
    %output = "tosa.conv2d"(%input, %filter, %zero, %input_zp, %weight_zp) {rock.arch = "gfx906", acc_type = i32, dilation = array<i64: 1, 1>, pad = array<i64: 0, 0, 0, 0>, stride = array<i64: 1, 1>} : (tensor<1x8x8x4xi8>, tensor<8x1x1x4xi8>, tensor<8xi8>, tensor<1xi8>, tensor<1xi8>) -> tensor<1x8x8x8xi32>

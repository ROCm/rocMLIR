    ...
    %bias: tensor<8xi32>) -> tensor<1x8x8x8xf32> attributes {kernel}
{
    ...
    %output = "tosa.conv2d"(%input, %filter, %zero, %input_zp, %weight_zp) {quantization_info = #tosa.conv_quant<input_zp = 0, weight_zp = 0>, arch = "gfx906", acc_type = i32, dilation = array<i64: 1, 1>, pad = array<i64: 0, 0, 0, 0>, stride = array<i64: 1, 1>} : (tensor<1x8x8x4xi8>, tensor<8x1x1x4xi8>, tensor<8xi8>, tensor<1xi8>, tensor<1xi8>) -> tensor<1x8x8x8xi32>
    ...
    %bias: tensor<8xi32>) -> tensor<1x8x8x8xf32> attributes {kernel}
{
    ...
    %const_shape = "tosa.const_shape"() { value = dense<[1, 1, 1, 8]> : tensor<4xindex> } : () -> !tosa.shape<4>

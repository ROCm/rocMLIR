// RUN: rocmlir-gen -emit-module-fusibility-for=v3:16,16,4,16,16,1,5,1,2 - < %s | FileCheck %s --check-prefixes=CHECK-SPLITK
// CHECK-SPLITK: fusible:0
// RUN: rocmlir-gen -emit-module-fusibility-for=v3:16,16,4,16,16,1,1,1,2 - < %s | FileCheck %s --check-prefixes=CHECK-NONSPLITK
// CHECK-NONSPLITK: fusible:1
module {
  func.func @mlir_dot_add(%arg0: memref<1x2x320xf32>, %arg1: memref<1x2x1280xf32>, %arg2: memref<1x1280x320xf32>, %arg3: memref<1x2x320xf32>) attributes {rock.enable_splitk_for_tuning, rock.kernel, mhal.arch = "amdgcn-amd-amdhsa:gfx1010:sramecc+:xnack-"} {
    %alloc = memref.alloc() alignment = 64 : memref<1x2x320xf32>
    rock.gemm %alloc = %arg1 * %arg2 storeMethod =  set {rock.arch = "amdgcn-amd-amdhsa:gfx1010:sramecc+:xnack-"} : memref<1x2x320xf32> = memref<1x2x1280xf32> * memref<1x1280x320xf32>

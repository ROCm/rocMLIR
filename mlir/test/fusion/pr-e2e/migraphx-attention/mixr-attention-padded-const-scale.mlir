...
// migraphx.attention variant of pr-e2e/attention/mixr-attention-padded-const-scale.mlir.
// The original test used a per-tensor scalar literal scale (1/8) broadcast to
// the QK shape via migraphx.multibroadcast. For migraphx.attention the
// preSoftmaxElemWiseInputs must already match the QK shape, and we can't
// keep a broadcast layout (`migraphx.mlir.as.underlying.shape` rejects
// writing to broadcast tensors), so the literal is materialised at full
// QK shape directly.
module {
  func.func private @mlir_attention(%arg0: !migraphx.shaped<1x7x3xf32, 21x3x1>,
    ...
    %scale = migraphx.literal (dense<1.250000e-01> : tensor<1x7x7xf32>)
      : <1x7x7xf32, 49x7x1>

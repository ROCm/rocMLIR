// RUN: rocmlir-gen -fut dot_max_special --arch %arch --clone-harness %s | rocmlir-driver -kernel-pipeline=migraphx,highlevel -host-pipeline=migraphx,highlevel | rocmlir-gen -ph -print-results -rand none -fut dot_max_special_wrapper - | rocmlir-driver -host-pipeline mhal,runner -kernel-pipeline full -targets %arch | xmir-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext,%linalg_test_lib_dir/libmlir_c_runner_utils%shlibext --entry-point-result=void | FileCheck %s
// RUN: rocmlir-gen -fut dot_max_special --arch %arch --clone-harness %s | rocmlir-driver -kernel-pipeline=migraphx-linalg,highlevel -host-pipeline=migraphx-linalg,highlevel | rocmlir-gen -ph -print-results -rand none -fut dot_max_special_wrapper - | rocmlir-driver -host-pipeline mhal,runner -kernel-pipeline full -targets %arch | xmir-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext,%linalg_test_lib_dir/libmlir_float16_utils%shlibext,%linalg_test_lib_dir/libmlir_c_runner_utils%shlibext --entry-point-result=void | FileCheck %s

// Verify NaN propagation from both operand positions, +0 over -0 in both
// orders, infinities, and ordinary finite values.
// CHECK: [nan,  nan,  0,  0,  inf,  inf,  0,  0,  0,  0,  1,  1,  42,  2,  3]
...
module {
  func.func @dot_max_special(
    ...
    %lhs_literal = migraphx.literal(
        dense<[[[0x7FC00000, 0.0, -0.0],
                 [0.0, 0x7F800000, 0.0],
                 [0xFF800000, 0.0, -1.0],
                 [0.0, 1.0, 0.0],
                 [42.0, -2.0, 3.0]]]> : tensor<1x5x3xf32>)
        : <1x5x3xf32, 15x3x1>
    %rhs_literal = migraphx.literal(
        dense<[[[0.0, 0x7FC00000, 0.0],
                 [-0.0, 0.0, 0x7F800000],
                 [0.0, 0xFF800000, 0.0],
                 [-1.0, 0.0, 1.0],
                 [-1.0, 2.0, 3.0]]]> : tensor<1x5x3xf32>)
        : <1x5x3xf32, 15x3x1>

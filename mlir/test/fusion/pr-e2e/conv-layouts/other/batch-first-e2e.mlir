...
// EMITKEY: convfp16 -F 1 -f N01GC -I 10CNG -O NGC01 -n 2 -c 16 -H 160 -W 160 -k 16 -y 3 -x 3 -p 1 -q 1 -u 1 -v 1 -l 1 -j 1 -g 1
...
module {
  func.func @test(%arg0: !migraphx.shaped<2x32x160x160xf16, 1x2x64x10240>, %arg1: !migraphx.shaped<16x16x3x3xf16, 144x1x48x16>, %arg2: !migraphx.shaped<2x16x160x160xf16, 0x1x0x0>) -> !migraphx.shaped<2x16x160x160xf16, 409600x1x2560x16> {
    %0 = migraphx.slice %arg0 {axes = [1], ends = [32], starts = [16]} : <2x32x160x160xf16, 1x2x64x10240> -> <2x16x160x160xf16, 1x2x64x10240>
    %1 = migraphx.convolution %0, %arg1 {dilation = [1, 1], group = 1 : i64, padding = [1, 1, 1, 1], padding_mode = 0 : i64, stride = [1, 1]} : <2x16x160x160xf16, 1x2x64x10240>, <16x16x3x3xf16, 144x1x48x16> -> <2x16x160x160xf16, 409600x1x2560x16>

// RUN: rock-gen -p -fil_layout=gyxck -in_layout=hwgcn -out_layout=hwgkn --host %s | FileCheck %s --check-prefix=HARNESS
// RUN: rock-gen -p -fil_layout=gyxck -in_layout=hwgcn -out_layout=hwgkn --host %s | mlir-rock-driver -c | FileCheck %s --check-prefix=LOWERING
// RUN: rock-gen -p -fil_layout=gyxck -in_layout=hwgcn -out_layout=hwgkn --host %s | mlir-rock-driver -c | mlir-rocm-runner --shared-libs=%linalg_test_lib_dir/libmlir_rocm_runtime%shlibext,%conv_validation_wrapper_library_dir/libconv-validation-wrappers%shlibext,%linalg_test_lib_dir/libmlir_runner_utils%shlibext --entry-point-result=void | FileCheck %s --check-prefix=E2E

...
// CHECK: func.func private @mlir_reshape_convolution({{.*}}: memref<256xf32> {mhal.read_access}, {{.*}}: memref<9xf32> {mhal.read_access}, {{.*}}: memref<1024xf32> {mhal.write_access}) attributes {original_func = @mlir_reshape_convolution, rock.kernel}
// CHECK-LABEL: @main

...
def usage():
    print(
        f"usage: python3 {sys.argv[0]} [-h | -d <directory> | -i <configfile> | -s]"
    )
...
if __name__ == '__main__':
    ...
    for suite in toml_dict["suite"]:
        ...
        for test in suite["test"]:
            ...
            exclusions = []
            if "exclude" in test:
                exclusions = generate_option_list(axis_prefixes, test,
                                                  "exclude", "values")
            for opt in combinations:
                # Only generate i8 data type for fwd convolutions
                if len(opt) >= 3 and "i8" in opt[2] and (
                        "conv_bwd_data" in opt[0] or
                ...
                output = output_path + '/' + suite_name + '_' + str(
                    test_idx) + '_' + str(opt_idx) + '.mlir'
                with open(output, 'w') as f:
                    ...
                    f.write(
                        f"{config}  {suffix}CHECK_{suite_name}_{test_idx}_{opt_idx}"
                    )
                    if single_mode:
                        f.write("\n")
                    f.write(f"""
...
""")
            ...
            if not single_mode:
                test_files = glob.glob(output_path + '/' + suite_name + '_' +
                                       str(test_idx) + '_*' + '.mlir')
                merged_file = output_path + '/' + suite_name + '_' + str(
                    test_idx) + '.mlir'

...
namespace {
struct ByIndices {
  static SmallVector<int64_t> getRemovees(func::FuncOp f) {
    SmallVector<int64_t> removeIndices;
    auto attrArray = f->getAttr("remove_dims_by_indices");
    for (auto &attr : attrArray.cast<ArrayAttr>()) {
      removeIndices.push_back(attr.cast<IntegerAttr>().getInt());
    }
...
struct ByNames {
  static SmallVector<StringRef> getRemovees(func::FuncOp f) {
    SmallVector<StringRef> removeIndices;
    auto attrArray = f->getAttr("remove_dims_by_names");
    for (auto &attr : attrArray.cast<ArrayAttr>()) {
      removeIndices.push_back(attr.cast<StringAttr>().getValue());
    }
    return removeIndices;
  }
};

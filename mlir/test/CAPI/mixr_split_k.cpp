...
// RUN: mlir-mixr-split-k-test -t=DataType::F32 use-ew-op=false  2>&1 |
// FileCheck %s --check-prefix=F32_EW_OFF
...
static bool constructAndTraverseIr(MlirContext ctx,
  ...
  RocmlirSplitKSelectionLikelihood likelihood = mlirIsSplitKFaster(
      options.M, options.N, options.K, numCUs, options.tuningLevel);
  std::cout << "splitk selection likelihood: " << likelihood << std::endl;
  // F32_EW_OFF: splitk selection likelihood: maybe
  ...
  // F32_EW_OFF: prefill arg indices: 2

  std::cout << "prefill arg init values: " << prefillArgValues << '\n';
  // F32_EW_OFF: prefill arg init values: 0.00
  ...
  // F32_EW_OFF: aux buffers sizes:

  std::cout << "aux buffers init values: " << auxBuffersInitValues << '\n';
  // F32_EW_OFF: aux buffers init values:
...
llvm::cl::opt<bool> useElementwiseOp(
    "use-ew-op",
    llvm::cl::desc("append elementwise operaion at the end of `dot`"),
    llvm::cl::init(false));

...
module attributes {gpu.container_module} {
  ...
  gpu.module @miopen_kernel_module {
    gpu.func @miopen_conv2d_kcyx_nchw_nkhw(%arg0: memref<128x8x3x3xf32>, %arg1: memref<128x8x32x32xf32>, %arg2: memref<128x128x30x30xf32>) kernel {
      ...
      %54 = miopen.alloc() : memref<1x8xf32, 5>  // TBD: is the dimension really correct?
      ...
      scf.for %arg3 = %c0 to %c1 step %c1 {      // just do 1 iteration as the LDS has only been populated once.
        ...
        scf.for %arg4 = %c0 to %c1 step %c1 {    // just do 1 iteration
          ...
          miopen.threadwise_copy(%20, %54, %56, %60, %c0_i32, %62) {coord_transforms = [{operand = 0 : i32, transforms = [affine_map<(d0, d1) -> (d1 + d0 * 128)>]}], data_per_access = 1 : i32, n_slice_col = 4 : i32, n_slice_row = 1 : i32} : memref<4096xf32, 3>, memref<1x8xf32, 5>
        }
        ...
        scf.for %arg4 = %c0 to %c1 step %c1 {   // NPerThread (8) / NPerThreadSubC (4) = NRepeat (2) iterations
          ...
          // XXX populate the later half othe buffer.
          %62 = addi %61, %c4_i32 : i32
          miopen.threadwise_copy(%20, %55, %56, %60, %c0_i32, %62) {coord_transforms = [{operand = 0 : i32, transforms = [affine_map<(d0, d1) -> (d1 + d0 * 128 + 2048)>]}], data_per_access = 1 : i32, n_slice_col = 4 : i32, n_slice_row = 1 : i32} : memref<4096xf32, 3>, memref<1x8xf32, 5>
        }

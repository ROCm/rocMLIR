...
static void affixThreadwiseCopyAttributes(T top,
                                          U bop,
                                          OpBuilder &b,
...
struct GridwiseGemmRewritePattern : public OpRewritePattern<miopen::GridwiseGemmOp> {
  ...
  template <typename T>
  void affixBlockwiseCopyAttributes(T bop,
  ...
  LogicalResult matchAndRewrite(miopen::GridwiseGemmOp op, PatternRewriter &b) const override {
    ...
    auto blockwiseLoadALength =
	    GemmABlockCopyThreadSliceLengths_GemmK *
	    GemmABlockCopyThreadSliceLengths_GemmM;
    Type blockwiseLoadAType = (blockwiseLoadALength > 1) ?
	    VectorType::get(blockwiseLoadALength, elementType) :
	    elementType;

    auto blockwiseLoadBLength =
	    GemmBBlockCopyThreadSliceLengths_GemmK *
	    GemmBBlockCopyThreadSliceLengths_GemmN;
    Type blockwiseLoadBType = (blockwiseLoadALength > 1) ?
    ...
    auto blockwiseStoreA = b.create<miopen::BlockwiseStoreOp>(
        loc, blockwiseLoadA.getResult(), lds2DMatrixAEvenSubviewOp,
	blockwiseCopyADstVector);
    ...
    auto blockwiseStoreB = b.create<miopen::BlockwiseStoreOp>(
        loc, blockwiseLoadB.getResult(), lds2DMatrixBEvenSubviewOp,
	blockwiseCopyBDstVector);
    ...
    // Emit blockwise_load for matrix A.
    auto blockwiseLoadAEven = lb.create<miopen::BlockwiseLoadOp>(
        loc, blockwiseLoadAType, op.filter(), blockwiseCopyASrcVectorUpdated);
    affixBlockwiseCopyAttributes(blockwiseLoadAEven, op, b,
                                 /*isMatrixA=*/true);
    ...
    // Emit blockwise_load for matrix B.
    auto blockwiseLoadBEven = lb.create<miopen::BlockwiseLoadOp>(
        loc, blockwiseLoadBType, op.input(), blockwiseCopyBSrcVectorUpdated);
    affixBlockwiseCopyAttributes(blockwiseLoadBEven, op, b,
                                 /*isMatrixA=*/false);
    ...
    // Emit blockwise_store for matrix A.
    auto blockwiseStoreAOdd = lb.create<miopen::BlockwiseStoreOp>(
        loc, blockwiseLoadAEven.getResult(), lds2DMatrixAOddSubviewOp,
	loopOp.getRegionIterArgs()[1]);
    affixBlockwiseCopyAttributes(blockwiseStoreAOdd, op, b,
                                 /*isMatrixA=*/true);

    // Emit blockwise_store for matrix B.
    auto blockwiseStoreBOdd = lb.create<miopen::BlockwiseStoreOp>(
        loc, blockwiseLoadBEven.getResult(), lds2DMatrixBOddSubviewOp,
	loopOp.getRegionIterArgs()[3]);
    affixBlockwiseCopyAttributes(blockwiseStoreBOdd, op, b,
                                 /*isMatrixA=*/false);
    ...
    // Emit blockwise_load for matrix A.
    auto blockwiseLoadAOddSecondIteration = lb.create<miopen::BlockwiseLoadOp>(
        loc, blockwiseLoadAType, op.filter(), blockwiseCopyASrcVectorUpdated);
    affixBlockwiseCopyAttributes(blockwiseLoadAOddSecondIteration, op, b,
                                 /*isMatrixA=*/true);
    ...
    // Emit blockwise_load for matrix B.
    auto blockwiseLoadBOddSecondIteration = lb.create<miopen::BlockwiseLoadOp>(
        loc, blockwiseLoadBType, op.input(), blockwiseCopyBSrcVectorUpdated);
    affixBlockwiseCopyAttributes(blockwiseLoadBOddSecondIteration, op, b,
                                 /*isMatrixA=*/false);
    ...
    // Emit blockwise_store for matrix A.
    auto blockwiseStoreAEvenSecondIteration = lb.create<miopen::BlockwiseStoreOp>(
        loc, blockwiseLoadAOddSecondIteration.getResult(), lds2DMatrixAEvenSubviewOp,
	loopOp.getRegionIterArgs()[1]);
    affixBlockwiseCopyAttributes(blockwiseStoreAEvenSecondIteration, op, b,
                                 /*isMatrixA=*/true);

    // Emit blockwise_store for matrix B.
    auto blockwiseStoreBEvenSecondIteration = lb.create<miopen::BlockwiseStoreOp>(
        loc, blockwiseLoadBOddSecondIteration.getResult(), lds2DMatrixBEvenSubviewOp,
	loopOp.getRegionIterArgs()[3]);
    affixBlockwiseCopyAttributes(blockwiseStoreBEvenSecondIteration, op, b,
                                 /*isMatrixA=*/false);
...
struct BlockwiseLoadRewritePattern : public OpRewritePattern<miopen::BlockwiseLoadOp> {
  using OpRewritePattern<miopen::BlockwiseLoadOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(miopen::BlockwiseLoadOp op, PatternRewriter &b) const override {
    auto loc = op.getLoc();
    MemRefType sourceType = op.source().getType().cast<MemRefType>();
...
struct BlockwiseStoreRewritePattern : public OpRewritePattern<miopen::BlockwiseStoreOp> {
  using OpRewritePattern<miopen::BlockwiseStoreOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(miopen::BlockwiseStoreOp op, PatternRewriter &b) const override {
    auto loc = op.getLoc();
...
struct MovePosV2RewritePattern : public OpRewritePattern<miopen::MovePosV2Op> {
  ...
  LogicalResult matchAndRewrite(miopen::MovePosV2Op op,
    ...
    for (unsigned i = 0; i < vectorType.getShape()[0]; ++i) {
      ...
      auto element = b.create<vector::ExtractElementOp>(loc, vectorType.getElementType(), vector, iter);
      // add
      Value add;
      ...
      vector = b.create<vector::InsertElementOp>(loc, vectorType, add, vector, iter);
    }
...
struct ThreadwiseCopyRewritePattern
  ...
  LogicalResult matchAndRewrite(miopen::ThreadwiseCopyOp op,
    ...
    if (NSliceRowAttr && NSliceColAttr && DataPerAccessAttr) {
      ...
      for (unsigned ivo = 0; ivo < NSliceRow; ++ivo) {
        ...
        for (unsigned ivi = 0; ivi < NSliceCol; ivi += DataPerAccess) {
          ...
          if (DataPerAccess > 1) {
            ...
            b.create<vector::TransferWriteOp>(loc, vectorValue, op.dest(),
      ...
      if (!legacyLoadAttr || !legacyLoadAttr.template cast<BoolAttr>().getValue()) {
        // Compute low-level coordinate for source memref from sourceCoord.
        // Apply affine transformations to compute the low-level coordinate.
        for (unsigned i = 0; i < sourceCoordLength; ++i) {
          srcUpperCoord.push_back(b.create<IndexCastOp>(loc, sourceAndDestCoord[i], b.getIndexType()));
        }
        if (sourceExternalTransform || sourceEmbeddedTransform)
          srcLowerCoord = expandAffineMap(b, loc, sourceTransform, srcUpperCoord)
                                .getValue();
        else
      ...
      if (!legacyStoreAttr || !legacyStoreAttr.template cast<BoolAttr>().getValue()) {
        // Compute low-level coordinate for source memref from sourceCoord.
        // Apply affine transformations to compute the low-level coordinate.
        for (unsigned i = sourceCoordLength;
             i < sourceCoordLength + destCoordLength; ++i) {
          destUpperCoord.push_back(b.create<IndexCastOp>(loc, sourceAndDestCoord[i], b.getIndexType()));
        }
        if (destExternalTransform || destEmbeddedTransform)
          destLowerCoord = expandAffineMap(b, loc, destTransform, destUpperCoord)
                                .getValue();
        else
      ...
      do {
        ...
        if (!legacyLoadAttr || (legacyLoadAttr && !legacyLoadAttr.template cast<BoolAttr>().getValue())) {
          // -----------------------
          // New logic.
          // -----------------------
          ...
          SmallVector<Value, 4> srcIndexLowerNewUpdated;
          {
            ...
            for (int64_t iter = 0; iter < sourceType.getShape().size(); ++iter) {
              Value v = b.create<AddIOp>(loc, srcLowerCoord[iter], indexLowerDiffTmpOp[iter]);
              // v.dump();
          ...
          if (srcDataPerRead > 1) {
            ...
            // Issue scalar load.
            scalarValue = b.create<LoadOp>(loc, sourceType.getElementType(), op.source(), srcIndexLowerNewUpdated);
          }
        } // if (!legacyLoadAttr || !legacyLoadAttr.template cast<BoolAttr>().getValue()) {
        ...
        if (!legacyStoreAttr || (legacyStoreAttr && !legacyStoreAttr.template cast<BoolAttr>().getValue())) {
          ...
          SmallVector<Value, 4> destIndexLowerNewUpdated;
          {
            ...
            for (int64_t iter = 0; iter < destType.getShape().size(); ++iter) {
              Value v = b.create<AddIOp>(loc, destLowerCoord[iter], indexLowerDiffTmpOp[iter]);
              // v.dump();
              indexLowerNew.push_back(v);
            }
            // llvm::errs() << "\n";
...
struct ThreadwiseLoadRewritePattern
  ...
  LogicalResult matchAndRewrite(miopen::ThreadwiseLoadOp op,
    ...
    // Apply affine transformations to compute the low-level coordinate.
    for (unsigned i = 0; i < sourceCoordLength; ++i) {
      srcUpperCoord.push_back(b.create<IndexCastOp>(loc, sourceCoord[i], b.getIndexType()));
    }
    if (sourceExternalTransform || sourceEmbeddedTransform)
      srcLowerCoord = expandAffineMap(b, loc, sourceTransform, srcUpperCoord)
                            .getValue();
	...
	for (auto v : srcIndexLowerNewUpdated)
          srcIndexLowerNewUpdatedI32.push_back(b.create<IndexCastOp>(loc, v, b.getIntegerType(32)));
	loadedValue = b.create<gpu::MubufLoadOp>(loc, resultType, op.source(), srcIndexLowerNewUpdatedI32);
      // } else {
      //   // Issue scalar load.
      //   loadedValue = b.create<LoadOp>(loc, dataType, op.source(), srcIndexLowerNewUpdated);
      // }
...
struct ThreadwiseStoreRewritePattern
  ...
  LogicalResult matchAndRewrite(miopen::ThreadwiseStoreOp op,
    ...
    for (unsigned i = 0; i < destCoordLength; ++i) {
      destUpperCoord.push_back(b.create<IndexCastOp>(loc, destCoord[i], b.getIndexType()));
    }
    if (destExternalTransform || destEmbeddedTransform)
      destLowerCoord = expandAffineMap(b, loc, destTransform, destUpperCoord)
                            .getValue();
    ...
    do {
      ...
      if (destDataPerWrite > 1) {
        ...
        if (valueType.isa<VectorType>()) {
          ...
          auto iter = b.create<ConstantIntOp>(loc, loopIVsPerAccessOrder[loopIVsPerAccessOrder.size() - 1], b.getIntegerType(32));
          auto element = b.create<vector::ExtractElementOp>(loc, dataType, op.data(), iter);
          b.create<StoreOp>(loc, element, op.dest(), destIndexLowerNewUpdated);
	} else {
          // Issue scalar store.
          b.create<StoreOp>(loc, op.data(), op.dest(), destIndexLowerNewUpdated);
        }
      }
...
struct ThreadwiseCopyV2RewritePattern
  ...
  LogicalResult matchAndRewrite(miopen::ThreadwiseCopyV2Op op,
    ...
    auto srcPosition = b.create<IndexCastOp>(loc, 
                         b.create<AddIOp>(loc, srcLowerCoord[0],
                           b.create<IndexCastOp>(loc, op.sourceOffset(), b.getIndexType())),
                         b.getIntegerType(32));
    ...
    do {
      ...
      SmallVector<Value, 4> srcIndexLowerNewUpdated;
      {
        ...
        // llvm::errs() << "index lower new before borrow/carry:\n";
        for (int64_t iter = 0; iter < sourceType.getShape().size(); ++iter) {
          Value v = b.create<AddIOp>(loc, b.create<IndexCastOp>(loc, srcLowerCoord[iter], b.getIntegerType(32)),
      ...
      if (srcDataPerRead > 1) {
        ...
        // Issue scalar load.
        scalarValue = b.create<vector::ExtractElementOp>(loc, sourceType.getElementType(), op.source(), srcIndexLowerNewUpdated[0]);
      }
      ...
      SmallVector<Value, 4> destIndexLowerNewUpdated;
      {
        ...
        // llvm::errs() << "index lower new before borrow/carry:\n";
        for (int64_t iter = 0; iter < destType.getShape().size(); ++iter) {
          Value v = b.create<AddIOp>(loc, b.create<IndexCastOp>(loc, destLowerCoord[iter], b.getIntegerType(32)),
                                     indexLowerDiffTmpOp[iter]);
          // v.dump();
          indexLowerNew.push_back(v);
        }

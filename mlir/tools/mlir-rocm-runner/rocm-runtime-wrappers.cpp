...
mcpuConv2d(float *filterAllocated, float *filterAligned, int64_t filterOffset,
  ...
  llvm::SmallVector<int64_t, 4> filterSizes({filterSize0, filterSize1, filterSize2, filterSize3});
  llvm::SmallVector<int64_t, 4> filterStrides({filterStride0, filterStride1, filterStride2, filterStride3});
  llvm::SmallVector<int64_t, 4> inputSizes({inputSize0, inputSize1, inputSize2, inputSize3});
  llvm::SmallVector<int64_t, 4> inputStrides({inputStride0, inputStride1, inputStride2, inputStride3});
  llvm::SmallVector<int64_t, 4> outputSizes({outputSize0, outputSize1, outputSize2, outputSize3});
  llvm::SmallVector<int64_t, 4> outputStrides({outputStride0, outputStride1, outputStride2, outputStride3});
                ...
                if (in_h < 0 || in_h >= inHeightSize || in_w < 0 ||
                  ...
                  input =
                      inputAllocated[n * inBatchStride + c * inChannelStride +
                                     in_h * inHeightStride + in_w* inWidthStride];

                outputAllocated[n * outBatchStride + k * outChannelStride +
                                out_h * outHeightStride + out_w * outWidthStride] +=
                    input *
                    filterAllocated[k * filterOutChanStride +
                                    c * filterInChanStride + fil_h * filterHeightStride +

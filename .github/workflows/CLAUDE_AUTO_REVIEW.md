...
| `.github/workflows/claude_auto_review_coding_standards_sync.yml` | Enforces byte-for-byte sync between `docs/CODING_STANDARDS.md` and the inlined copy in the review prompt heredoc (see §15). |
...
| **Overlay trusted `.claude/` + `.github/scripts/` + `docs/CODING_STANDARDS.md`** | Restores the default-branch versions into the workspace so the skills, sanitizer, and coding-standards reference that actually *drive* the review are the trusted ones. Defense-in-depth (a PR could remove this step, but that's visible in the diff Layer 2 audits). `docs/CODING_STANDARDS.md` is overlaid in addition to being inlined into the prompt (see [§15](#15-maintenance--sync-points)) — belt-and-braces against working-tree drift on PRs opened before the file existed on disk. |
...
| Trusted overlay of skills + scripts + coding standards | Accidental (or intentional) in-PR changes to review logic, sanitizer logic, or the coding-standards tier list don't take effect at runtime. The standards are also inlined into the prompt — see [§15](#15-maintenance--sync-points). |
...
| Coding-standards content | `docs/CODING_STANDARDS.md` (the single contributor-facing source) AND the same content inlined verbatim into the prompt heredoc inside `claude_auto_review.yml` (the `## Coding standards (canonical reference)` section, between the `<BEGIN docs/CODING_STANDARDS.md>` / `<END docs/CODING_STANDARDS.md>` markers, body verbatim minus the H1 title). When you edit one you MUST paste the same content into the other. The workflow also overlays the file from the default branch into the workspace (see [§8 Job 1](#job-1--review-has-secrets-no-model-accessible-write-token)) so `Read('docs/CODING_STANDARDS.md')` returns the same bytes -- belt-and-braces against working-tree drift. **CI gate**: the dedicated `claude_auto_review_coding_standards_sync.yml` workflow (path-filtered to `docs/CODING_STANDARDS.md` + `claude_auto_review.yml`) runs the verification script on every PR that touches either file and blocks merge on drift -- so the manual ritual below is now a sanity check, not the only guard. To verify locally before pushing: `python3 -c "import re, yaml; wf=yaml.safe_load(open('.github/workflows/claude_auto_review.yml')); p=[s for s in wf['jobs']['review']['steps'] if s.get('uses','').startswith('anthropics/claude-code-action')][0]['with']['prompt']; m=re.search(r'<BEGIN docs/CODING_STANDARDS\.md>\n(.*?)\n\s*<END docs/CODING_STANDARDS\.md>', p, re.S); f=open('docs/CODING_STANDARDS.md').read(); assert m and m.group(1).strip() == '\n'.join(f.splitlines()[2:]).strip(), 'DRIFT'; print('in-sync')"`. |

...
| **Snapshot trusted coding standards → step output** | Reads the (already-overlaid, trusted) `docs/CODING_STANDARDS.md`, drops its H1 + the following blank, and writes the remainder to `$GITHUB_OUTPUT` (`steps.snapshot_standards.outputs.content`). The next step's prompt heredoc substitutes that output between `<BEGIN/END docs/CODING_STANDARDS.md>` markers, making the file the **single source of truth** for what the model sees -- no static inline duplicate exists, so drift between "the file" and "the prompt" is impossible by construction. |

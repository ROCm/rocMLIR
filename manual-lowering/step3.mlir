// Manual lowering using RockptrToArith pass
// Goal: 
// 1. Lower transforms_to_ptr into pointer arithmetic
// 2. Convert blockwise_load_ptr allocs into results
...
module {
  func.func @rock_gemm(%arg0: memref<1179648xf16>, %arg1: memref<2359296xf16>, %arg2: memref<294912xf32>) attributes {arch = "amdgcn-amd-amdhsa:gfx950:sramecc+:xnack-", block_size = 256 : i32, enable_splitk_for_tuning, features = #rock<GemmFeatures mfma|dot|atomic_add|atomic_add_bf16|atomic_add_f16|direct_to_lds_32b|direct_to_lds_128b>, grid_size = 144 : i32, kernel, num_chiplets = 8 : i64, num_cu = 256 : i64, waves_per_eu = 0 : i64} {
    ...
    // Step 1. Because we need to perform pointer arithmetic, we need to extract the pointers from the memrefs.
    %ptrA_raw = memref.extract_aligned_pointer_as_index %arg0 : memref<1179648xf16> -> index
    %ptrB_raw = memref.extract_aligned_pointer_as_index %arg1 : memref<2359296xf16> -> index
    %ptrC_raw = memref.extract_aligned_pointer_as_index %arg2 : memref<294912xf32> -> index

    // [OLD IR] %0 = rock.transform %arg0 by #transform_map : memref<1179648xf16> to memref<1x384x3072xf16>
    // [OLD IR] %1 = rock.transform %arg1 by #transform_map1 : memref<2359296xf16> to memref<1x3072x768xf16>
    // [OLD IR] %2 = rock.transform %arg2 by #transform_map2 : memref<294912xf32> to memref<1x384x768xf32>
    // [OLD IR] %3 = rock.transform %0 by #transform_map3 : memref<1x384x3072xf16> to memref<1x3072x384xf16>
    ...
    // Define strides for A, B
    %stride_am = arith.constant 1 : i32
    %stride_ak = arith.constant 1 : i32
    %stride_bk = arith.constant 1 : i32
    %stride_bn = arith.constant 1 : i32

    // Splats
    %a_ptrs_14 = tt.splat %stride_am : i32 -> tensor<64x1xi32>
    %a_ptrs_17 = tt.splat %stride_ak : i32 -> tensor<1x64xi32>
    %b_ptrs_24 = tt.splat %stride_bk : i32 -> tensor<64x1xi32>
    %b_ptrs_27 = tt.splat %stride_bn : i32 -> tensor<1x64xi32>    
    ...
    scf.for %arg3 = %c0 to %c48 step %c1 {
      ...
      // [OLD IR] %t25 = rock.transform %3 by #transform_map5 : memref<1x3072x384xf16> to memref<48x1x12x12x32x64xf16>

      // [OLD IR] %mask_tensor_a, %24 = rock.transforms_to_ptr %t24[%arg3, %11, %18, %20] : memref<48x1x12x12x64x64xf16> to memref<64x64xindex>, memref<64x64xi1>
      // [OLD IR] %mask_tensor_b, %25 = rock.transforms_to_ptr %t25[%arg3, %11, %18, %20] : memref<48x1x12x12x32x64xf16> to memref<32x64xindex>, memref<32x64xi1>

      // Step 3. Compute blockwise_load_ptr input buffers, which now are pointers, by doing pointer arithmetic.
      %a_ptr = rock.expand_dims %offs_a {axis = 1 : i32} : memref<64xi32> -> memref<64x1xi32>
      %a_ptrs_15 = arith.muli %a_ptrs, %a_ptrs_14 : tensor<64x1xi32> loc(#loc112)
      // Here it goes the IR to compute the pointer and the mask...
      // ... the output for A is %24 
      // ... the output for B is %25
      // ... it also generates %mask_tensor_a and %mask_tensor_b

      // Step 4. Make blockwise_load_ptr return the result.
      // This is a bit hacky because is bufferized, but it works good enough to represent that the loaded data
      // is returned into a new result, instead of written into a previously allocated buffer.
      // 
      // [OLD IR] rock.blockwise_load_ptr(%24, %mask_tensor_a) -> %22 : (memref<64x64xindex>, memref<64x64xi1>) -> memref<64x64xf16, #gpu.address_space<private>>
      // [OLD IR] rock.blockwise_load_ptr(%25, %mask_tensor_b) -> %21 : (memref<32x64xindex>, memref<32x64xi1>) -> memref<32x64xf16, #gpu.address_space<private>>
    ...
    // ... it also generates %mask_tensor_c

...
#transform_map = #rock.transform_map<
  #map by [
    <Unmerge{384, 3072} ["m", "k"] at [1, 2] -> ["raw"] at [0]>, 
    <AddDim{1} ["g"] at [0] -> [] at []>
  ] bounds = [1, 384, 3072] -> [1179648]>
...
module {
  func.func @rock_gemm(%arg0: memref<1179648xf16>, %arg1: memref<2359296xf16>, %arg2: memref<294912xf32>) attributes {arch = "amdgcn-amd-amdhsa:gfx950:sramecc+:xnack-", block_size = 256 : i32, enable_splitk_for_tuning, features = #rock<GemmFeatures mfma|dot|atomic_add|atomic_add_bf16|atomic_add_f16|direct_to_lds_32b|direct_to_lds_128b>, grid_size = 144 : i32, kernel, num_chiplets = 8 : i64, num_cu = 256 : i64, waves_per_eu = 0 : i64} {
    ...
    %buf1 = rock.alloc() : memref<64xf16, #gpu.address_space<private>>
    %range1 = rock.make_range %buf1, %c0, %c64 : memref<64xf16>
    %21 = tensor.splat %range1 : memref<64xf16> -> memref<64x32xf16>

    %buf2 = rock.alloc() : memref<64x64xf16, #gpu.address_space<private>>
    %range2 = rock.make_range %buf2, %c0, %c64 : memref<64x64xf16>
    %22 = tensor.splat %range2 : memref<64x64xf16> -> memref<64x64xf16>

    %23 = rock.alloc() : memref<32x64xf32, #gpu.address_space<private>>
    rock.fill(%23, %cst) : memref<32x64xf32, #gpu.address_space<private>>, f32
    
    // Step 2. Translate transforms into ptr_arith
    // %24 = rock.transform %1 by #transform_map4 : memref<1x3072x768xf16> to memref<48x1x12x12x64x64xf16>
    // %25 = rock.transform %3 by #transform_map5 : memref<1x3072x384xf16> to memref<48x1x12x12x32x64xf16>

    // Previous step had chained transforms, here we combine all transforms into one single op with 2 or 3 transforms:
    %24 = rock.ptr_arith %arg1 by #transform_map1, #transform_map4 : memref<2359296xf16> to ptr
    %25 = rock.ptr_arith %arg0 by #transform_map, #transform_map3, #transform_map5 : memref<1179648xf16> to ptr

    // Step3. Not sure how to determine those guys
    %mask_tensor_a = rock.mask_tensor %24 : ptr
    %mask_tensor_b = rock.mask_tensor %25 : ptr
    %mask_tensor_c = rock.mask_tensor %arg2 : ptr
    ...
    scf.for %arg3 = %c0 to %c48 step %c1 {
      ...
      // Step 4. load_tiles to load_ptr and store_tile to store_ptr
      rock.blockwise_load_ptr(%24, %mask_tensor_a) -> %22 : ptr -> memref<64x64xf16, #gpu.address_space<private>>
      rock.blockwise_load_ptr(%25, %mask_tensor_b) -> %21 : ptr -> memref<32x64xf16, #gpu.address_space<private>>|
    ...
    rock.blockwise_store_ptr(%23, %mask_tensor_c) -> %arg2 : memref<32x64xf32, #gpu.address_space<private>> -> ptr

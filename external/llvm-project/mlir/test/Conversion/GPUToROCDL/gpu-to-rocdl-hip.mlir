...
gpu.module @test_module {
  ...
  gpu.func @test_const_printf() {
    ...
    // CHECK: %[[DESC0:.*]] = llvm.call @__ockl_printf_begin({{.*}}) : (i64) -> i64
    // CHECK: %[[FORMATSTR:.*]] = llvm.mlir.addressof @[[$PRINT_GLOBAL0]] : !llvm.ptr<array<14 x i8>>
    // CHECK: %[[FORMATSTART:.*]] = llvm.getelementptr %[[FORMATSTR]][%[[CST0]], %[[CST0]]] : (!llvm.ptr<array<14 x i8>>, i64, i64) -> !llvm.ptr<i8>
  ...
  gpu.func @test_printf(%arg0: i32) {
    ...
    // CHECK: %[[CST0:.*]] = llvm.mlir.constant(0 : i64) : i64
    // CHECK: %[[DESC0:.*]] = llvm.call @__ockl_printf_begin({{.*}}) : (i64) -> i64
    // CHECK: %[[FORMATSTR:.*]] = llvm.mlir.addressof @[[$PRINT_GLOBAL1]] : !llvm.ptr<array<11 x i8>>
    // CHECK: %[[FORMATSTART:.*]] = llvm.getelementptr %[[FORMATSTR]][%[[CST0]], %[[CST0]]] : (!llvm.ptr<array<11 x i8>>, i64, i64) -> !llvm.ptr<i8>

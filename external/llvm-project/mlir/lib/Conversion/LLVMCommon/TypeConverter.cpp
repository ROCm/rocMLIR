...
Type LLVMTypeConverter::convertFunctionType(FunctionType type) {
  ...
  Type converted =
      convertFunctionSignature(type, /*isVariadic=*/false, conversion, options.useBarePtrCallConv);
...
Type LLVMTypeConverter::convertFunctionSignature(
    ...
    LLVMTypeConverter::SignatureConversion &result, bool useBarePtrCallConv) {
...
Type LLVMTypeConverter::convertCallingConventionType(Type type, bool useBarePtrCallConv) {
...
Type LLVMTypeConverter::packFunctionResults(TypeRange types, bool useBarePtrCallConv) {
...
LogicalResult mlir::barePtrFuncArgTypeConverter(LLVMTypeConverter &converter,
                                                Type type,
                                                SmallVectorImpl<Type> &result) {
  auto llvmTy = converter.convertCallingConventionType(type, true);
  if (!llvmTy)
    return failure();

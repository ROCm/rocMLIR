...
void outlineConvPartOps(Operation *convOp,
                        llvm::SetVector<Operation*> &secondOps,
  ...
  outlinedFunc->setAttr("sym_visibility", StringAttr::get("private", ctx));
...
//     return false;
  ...
  SmallVector<Type> lhsResultTypes(lhs->result_type_begin(), lhs->result_type_end());
  SmallVector<Type> rhsResultTypes(rhs->result_type_begin(), rhs->result_type_end());
...
public:
  ...
  void runOnFunction() override {
    ...
    auto callback = [&](tosa::Conv2DOp convOp) {
      ...
      llvm::SetVector<Operation *> secondOps;
      llvm::SetVector<Value> inputNodes(convOp->getOperands().begin(),
                                        convOp->getOperands().end());
      llvm::SetVector<Value> resultNodes(convOp->getResults().begin(),
...
void PostPartitionCollapsePass::runOnOperation() {
  ...
  module.walk([&] (CallOp call) {
                if (replacements.find(call.getCallee()) != replacements.end()) {
                  call.calleeAttr(FlatSymbolRefAttr::get(replacements[call.getCallee()],
                                                         module->getContext()));
                }
              });
}
...
void registerTosaPartitionPass() {
  PassRegistration<TosaPartitionPass>("tosa-partition", "tosa partitioning");
  PassRegistration<PostPartitionCollapsePass>("tosa-collapse", "merge identical tosa partitions");

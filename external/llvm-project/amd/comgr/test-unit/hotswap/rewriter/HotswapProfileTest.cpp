...
/// (hotswap/rewriter/internal.h). The profiler is always compiled and gated
/// only at run time (AMD_COMGR_TIME_STATISTICS), so these tests exercise both
/// the disabled and enabled per-rewrite sessions directly. See review on

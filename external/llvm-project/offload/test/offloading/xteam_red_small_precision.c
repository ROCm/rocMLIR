...
#pragma omp target teams distribute parallel for map(tofrom : sum1)            \
    reduction(+ : sum1)
  for (int j = 0; j < N; j = j + 1)
    sum1 += a[j];

#pragma omp target teams distribute parallel for map(tofrom : sum2)            \
    reduction(+ : sum2)
  for (int j = 0; j < N; j = j + 2)
    sum2 += b[j];

#pragma omp target teams distribute parallel for map(tofrom : sum3)            \
    reduction(+ : sum3)
  for (int j = 0; j < N; j = j + 2)
    sum3 += c[j];

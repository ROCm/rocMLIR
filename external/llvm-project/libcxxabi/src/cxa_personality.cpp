...
extern "C" EXCEPTION_DISPOSITION _GCC_specific_handler(PEXCEPTION_RECORD, void*, PCONTEXT, PDISPATCHER_CONTEXT,
...
static const __shim_type_info* get_shim_type_info(uint64_t ttypeIndex, const uint8_t* classInfo, uint8_t ttypeEncoding,
...
*/
#if defined(_LIBCXXABI_ARM_EHABI)
static bool exception_spec_can_catch(int64_t specIndex, const uint8_t* classInfo, uint8_t ttypeEncoding,
  ...
  specIndex = -specIndex;
  --specIndex;
  const void** temp = reinterpret_cast<const void**>(reinterpret_cast<uintptr_t>(classInfo) +
...
static void scan_eh_tab(scan_results& results, _Unwind_Action actions, bool native_exception,
      ...
      while (true) {
        ...
        if (ttypeIndex > 0) {
          ...
          if (actions & _UA_FORCE_UNWIND) {
            ...
            if (exception_spec_can_catch(ttypeIndex, classInfo, ttypeEncoding, excpType, adjustedPtr, unwind_exception,
                                         base)) {
...
#    endif
#  endif
    (int version, _Unwind_Action actions, uint64_t exceptionClass, _Unwind_Exception* unwind_exception,
...
#  if defined(__SEH__) && !defined(__USING_SJLJ_EXCEPTIONS__)
extern "C" _LIBCXXABI_FUNC_VIS EXCEPTION_DISPOSITION __gxx_personality_seh0(PEXCEPTION_RECORD ms_exc, void* this_frame,
...
extern "C" _LIBCXXABI_FUNC_VIS _Unwind_Reason_Code __gxx_personality_v0(_Unwind_State state,
  ...
  case _US_UNWIND_FRAME_STARTING:
    ...
    if (unwind_exception->barrier_cache.sp == _Unwind_GetGR(context, REG_SP)) {
      ...
      if (native_exception) {
        ...
        scan_eh_tab(results, static_cast<_Unwind_Action>(_UA_CLEANUP_PHASE | _UA_HANDLER_FRAME), native_exception,
    ...
    if (is_force_unwinding)
      scan_eh_tab(results, static_cast<_Unwind_Action>(_UA_CLEANUP_PHASE | _UA_FORCE_UNWIND), native_exception,
                  unwind_exception, context);
    else
...
__attribute__((noreturn)) _LIBCXXABI_FUNC_VIS void __cxa_call_unexpected(void* arg) {
...
// __gxx_personality_v0().
_LIBCXXABI_FUNC_VIS _Unwind_Reason_Code __xlcxx_personality_v1(int version, _Unwind_Action actions,

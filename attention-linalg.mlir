...
func.func @causal_mask_attention(%q: tensor<1024x768xf32>, %kt: tensor<768x1024xf32>, %v: tensor<1024x768xf32>) -> tensor<1024x768xf32> {
    %output_init = arith.constant dense<0.0> : tensor<1024x768xf32>
    %result = linalg.generic {
        indexing_maps = [affine_map<(i, j) -> (i, j)>],
        iterator_types = ["parallel", "parallel"]
    } outs(%output_init : tensor<1024x768xf32>) {
        ^bb0(%out: f32):
            // Step 1: Q @ K^T -> tensor<1024x1024xf32>
            %qkt_init = arith.constant dense<0.0> : tensor<1024x1024xf32>
            %qkt = linalg.matmul ins(%q, %kt : tensor<1024x768xf32>, tensor<768x1024xf32>) outs(%qkt_init : tensor<1024x1024xf32>) -> tensor<1024x1024xf32>

            // Step 2a: Extract column 0 from qkt as initial max values (avoids -inf)
            %max_empty = tensor.empty() : tensor<1024xf32>
            %col0 = linalg.generic {
                indexing_maps = [affine_map<(d0) -> (d0, 0)>, affine_map<(d0) -> (d0)>],
                iterator_types = ["parallel"]
            } ins(%qkt : tensor<1024x1024xf32>) outs(%max_empty : tensor<1024xf32>) {
            ...
            } ins(%qkt : tensor<1024x1024xf32>) outs(%col0 : tensor<1024xf32>) {
                ^bb0(%val: f32, %acc: f32):
            ...
            %exp_empty = tensor.empty() : tensor<1024x1024xf32>
            %exp_result = linalg.generic {
                indexing_maps = [affine_map<(m, n) -> (m, n)>, affine_map<(m, n) -> (m)>, affine_map<(m, n) -> (m, n)>],
                iterator_types = ["parallel", "parallel"]
            } ins(%qkt, %row_max : tensor<1024x1024xf32>, tensor<1024xf32>) outs(%exp_empty : tensor<1024x1024xf32>) {
            ...
            %sum_init = arith.constant dense<0.0> : tensor<1024xf32>
            %row_sum = linalg.generic {
                indexing_maps = [affine_map<(m, n) -> (m, n)>, affine_map<(m, n) -> (m)>],
                iterator_types = ["parallel", "reduction"]
            } ins(%exp_result : tensor<1024x1024xf32>) outs(%sum_init : tensor<1024xf32>) {
                ^bb0(%val: f32, %acc: f32):
            ...
            %softmax_empty = tensor.empty() : tensor<1024x1024xf32>
            %softmax = linalg.generic {
                indexing_maps = [affine_map<(m, n) -> (m, n)>, affine_map<(m, n) -> (m)>, affine_map<(m, n) -> (m, n)>],
                iterator_types = ["parallel", "parallel"]
            } ins(%exp_result, %row_sum : tensor<1024x1024xf32>, tensor<1024xf32>) outs(%softmax_empty : tensor<1024x1024xf32>) {
            ...
            %k_range = tensor.empty() : tensor<1024xf32>
            %dot = linalg.generic {
                indexing_maps = [affine_map<(k) -> (k)>, affine_map<(k) -> ()>],
                iterator_types = ["reduction"]
            } ins(%k_range : tensor<1024xf32>) outs(%acc_scalar : tensor<f32>) {
                ^bb0(%dummy: f32, %acc: f32):
                    %k = linalg.index 0 : index
                    %s_ik = tensor.extract %softmax[%i, %k] : tensor<1024x1024xf32>
                    %v_kj = tensor.extract %v[%k, %j] : tensor<1024x768xf32>
    ...
    func.return %result : tensor<1024x768xf32>
}
...
// 9:53 am
func.func @main() {
    %q  = arith.constant dense<1.0> : tensor<1024x768xf32>
    %kt = arith.constant dense<2.0> : tensor<768x1024xf32>
    %v  = arith.constant dense<3.0> : tensor<1024x768xf32>

    %result = func.call @causal_mask_attention(%q, %kt, %v)
        : (tensor<1024x768xf32>, tensor<768x1024xf32>, tensor<1024x768xf32>) -> tensor<1024x768xf32>

    // %result_init = tensor.empty() : tensor<1024x1024xf32>
    // %result_init_two = tensor.empty() : tensor<1024x768xf32>
    // %result_softmax = tensor.empty() : tensor<1024x1024xf32>
    // %q_kt =  linalg.matmul ins(%q, %kt : tensor<1024x768xf32>, tensor<768x1024xf32>) outs(%result_init : tensor<1024x1024xf32>) -> tensor<1024x1024xf32>
    // %softmax = linalg.softmax dimension(1) ins(%q_kt : tensor<1024x1024xf32>) outs(%result_softmax : tensor<1024x1024xf32>) -> tensor<1024x1024xf32>
    // %result  = linalg.matmul ins(%q_kt, %v : tensor<1024x1024xf32>, tensor<1024x768xf32>) outs(%result_init_two : tensor<1024x768xf32>) -> tensor<1024x768xf32>

    %unranked = tensor.cast %result : tensor<1024x768xf32> to tensor<*xf32>
